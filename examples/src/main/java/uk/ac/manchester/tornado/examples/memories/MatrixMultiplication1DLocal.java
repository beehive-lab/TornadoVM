...
public class MatrixMultiplication1DLocal {

    public static final int WARMING_UP_ITERATIONS = 5;
    ...
    public static void main(String[] args) {
        ...
        TornadoDevice newDev = TornadoRuntime.getTornadoRuntime().getDriver(0).getDevice(0);
        TaskSchedule t = new TaskSchedule("s0");

        if (prebuilt) {
            t.prebuiltTask("t0", "matrixMultiplication", kernelLocation, new Object[] { matrixA, matrixB, matrixC, size }, new Access[] { Access.READ, Access.READ, Access.READ_WRITE, Access.READ },
                    newDev, new int[] { size, size }).streamOut(matrixC);
        } else {
            t.task("t0", MatrixMultiplication1DLocal::matrixMultiplication, matrixA, matrixB, matrixC, size).streamOut(matrixC);
