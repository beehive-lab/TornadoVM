...
public class VectorDivNode extends FloatingNode implements LIRLowerable, VectorOp {
    ...
    public VectorDivNode(CUDAKind kind, ValueNode x, ValueNode y) {
        this(TYPE, kind, x, y);
    }

    protected VectorDivNode(NodeClass<? extends VectorDivNode> c, CUDAKind kind, ValueNode x, ValueNode y) {
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        gen.getLIRGeneratorTool().append(new AssignStmt(result, new CUDABinary.Expr(CUDABinaryOp.DIV, gen.getLIRGeneratorTool().getLIRKind(stamp), input1, input2)));
