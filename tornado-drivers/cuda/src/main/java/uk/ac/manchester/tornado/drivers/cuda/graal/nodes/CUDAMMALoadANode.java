...
@NodeInfo
public class CUDAMMALoadANode extends FixedWithNextNode implements LIRLowerable, MarkMMAFragment {

    public static final NodeClass<CUDAMMALoadANode> TYPE = NodeClass.create(CUDAMMALoadANode.class);

    @Input private ValueNode tile;
    @Input private ValueNode wmmaK;
    @OptionalInput private ValueNode byteOffset;

    public CUDAMMALoadANode(ValueNode tile, ValueNode wmmaK) { this(tile, wmmaK, null); }
    public CUDAMMALoadANode(ValueNode tile, ValueNode wmmaK, ValueNode byteOffset) {
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        Variable fragA = tool.newVariable(LIRKind.value(CUDAKind.MMA_FRAG_A_F16));
        Value off = (byteOffset == null) ? null : gen.operand(byteOffset);
        int rowStride = 32; // 16 f16 × 2 bytes
