 ...
 * memory location}.
 */
@NodeInfo(nameTemplate = "CUDAAtomicWrite#{p#location/s}")
public class CUDAWriteAtomicNode extends AbstractWriteNode implements LIRLowerableAccess {

    public static final NodeClass<CUDAWriteAtomicNode> TYPE = NodeClass.create(CUDAWriteAtomicNode.class);
    ...
    public CUDAWriteAtomicNode(AddressNode address, LocationIdentity location, ValueNode value, BarrierType barrierType, ValueNode acc, Stamp accStamp, JavaKind elementKind,
    ...
    protected CUDAWriteAtomicNode(NodeClass<? extends CUDAWriteAtomicNode> c, AddressNode address, LocationIdentity location, ValueNode value, BarrierType barrierType) {
    ...
    public CUDAStamp getStampInt() {
        return switch (operation) {
            case ADD -> new CUDAStamp(CUDAKind.ATOMIC_ADD_INT);
            case MUL -> new CUDAStamp(CUDAKind.ATOMIC_MUL_INT);
            default -> throw new RuntimeException("Operation for reduction not supported yet: " + operation);
        };
    }

    public CUDAStamp getStampFloat() {
        CUDAStamp oclStamp = null;
        switch (operation) {
            case ADD:
                oclStamp = new CUDAStamp(CUDAKind.ATOMIC_ADD_FLOAT);
                break;
            default:
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        // New CUDADriver nodes for atomic add
        CUDAStamp oclStamp = switch (elementKind) {
            case Int -> getStampInt();
            case Long ->
                // DUE TO UNSUPPORTED FEATURE IN INTEL CUDADriver PLATFORM
                new CUDAStamp(CUDAKind.ATOMIC_ADD_INT);
