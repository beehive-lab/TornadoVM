...
public class CUDALIRGenerator extends LIRGenerator {
    ...
    public CUDALIRGenerator(CoreProviders providers, LIRGenerationResult res) {
        super(new CUDALIRKindTool((CUDATargetDescription) providers.getCodeCache().getTarget()), new CUDAArithmeticTool() {
    ...
    protected void emitRangeTableSwitch(int lowKey, LabelRef defaultTarget, LabelRef[] targets, SwitchStrategy remainingStrategy, LabelRef[] remainingTargets, AllocatableValue key) {
        Logger.traceBuildLIR(Logger.BACKEND.CUDA, "emitRangeTableSwitch: key=%s, lowKey=%d", key, lowKey);
        // For CUDA, we generate a regular switch statement
