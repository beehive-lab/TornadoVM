...
import uk.ac.manchester.tornado.drivers.cuda.graal.nodes.CUDAFieldAddressArithmeticNode;
 ...
 * a {@code CUDADecompressedReadFieldNode}. If this is the case, the {@code TornadoAddressArithmeticNode}
 * is replaced by a {@code CUDAFieldAddressArithmeticNode}, which emits the absolute address, as
 * evaluated by the corresponding {@code CUDADecompressedReadFieldNode}.
 */
public class CUDAFieldCoopsAccessPhase extends Phase {
    ...
    protected void run(StructuredGraph graph) {
        for (CUDADecompressedReadFieldNode readDecompressedField : graph.getNodes().filter(CUDADecompressedReadFieldNode.class)) {
            for (TornadoAddressArithmeticNode tornadoAddressArithmeticNode : readDecompressedField.usages().filter(TornadoAddressArithmeticNode.class)) {
                CUDAFieldAddressArithmeticNode oclFieldAddressArithmetic = new CUDAFieldAddressArithmeticNode(readDecompressedField);
