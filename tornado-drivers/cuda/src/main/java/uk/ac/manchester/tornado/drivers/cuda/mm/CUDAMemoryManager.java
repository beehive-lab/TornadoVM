...
import static uk.ac.manchester.tornado.drivers.cuda.mm.CUDAKernelStackFrame.RESERVED_SLOTS;
...
public class CUDAMemoryManager implements TornadoMemoryProvider {
    ...
    private Map<Long, CUDAKernelStackFrame> oclKernelStackFrame = new ConcurrentHashMap<>();
    private long constantMemoryPointer;
    private long NON_EXISTING_ADDRESS = -1;
    private long atomicsRegionPointer = -1;

    public CUDAMemoryManager(final CUDADeviceContext deviceContext) {
    ...
    public void releaseKernelStackFrame(long executionPlanId) {
        CUDAKernelStackFrame stackFrame = oclKernelStackFrame.remove(executionPlanId);
        if (stackFrame != null) {
    ...
    public XPUBuffer createAtomicsBuffer(final int[] array, Access access) {
        return new CUDAAtomicsBuffer(array, deviceContext, access);
    }
    ...
    public void allocateDeviceMemoryRegions() {
        this.constantMemoryPointer = createBuffer(4, CUDAMemFlags.CL_MEM_READ_ONLY | CUDAMemFlags.CL_MEM_ALLOC_HOST_PTR).getBuffer();
