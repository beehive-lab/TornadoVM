...
public class CUDAMultiDimArrayWrapper<T, E> extends CUDAArrayWrapper<T> {

    private final Function<CUDADeviceContext, ? extends CUDAArrayWrapper<E>> innerWrapperFactory;
    private final CUDALongArrayWrapper tableWrapper;
    private long[] addresses;
    private CUDAArrayWrapper<E>[] wrappers;
    private final CUDADeviceContext deviceContext;
    private Access access;

    public CUDAMultiDimArrayWrapper(CUDADeviceContext device, Function<CUDADeviceContext, ? extends CUDAArrayWrapper<E>> factory, long batchSize, Access access) {
        super(device, JavaKind.Object, batchSize, access);
        this.deviceContext = device;
        this.access = access;
        innerWrapperFactory = factory;
        tableWrapper = new CUDALongArrayWrapper(device, batchSize, access);
    }
    ...
    public void allocate(Object value, long batchSize, Access access) throws TornadoOutOfMemoryException, TornadoMemoryException {
        ...
        wrappers = new CUDAArrayWrapper[Array.getLength(value)];
    ...
    public int getSizeOfType() {
        throw new TornadoRuntimeException("[ERROR] CUDAMultiDimArrayWrapper getSizeOfType not supported");
    }
