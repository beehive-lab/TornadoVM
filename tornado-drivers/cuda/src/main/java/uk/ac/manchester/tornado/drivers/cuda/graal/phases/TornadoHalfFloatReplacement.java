...
import uk.ac.manchester.tornado.drivers.cuda.graal.nodes.vector.VectorAddHalfNode;
import uk.ac.manchester.tornado.drivers.cuda.graal.nodes.vector.VectorLoadElementNode;
import uk.ac.manchester.tornado.drivers.cuda.graal.nodes.vector.VectorMultHalfNode;
import uk.ac.manchester.tornado.drivers.cuda.graal.nodes.vector.VectorSubHalfNode;
...
public class TornadoHalfFloatReplacement extends BasePhase<TornadoHighTierContext> {
    ...
    private static Node identifyFieldReplacement(Node input, ArrayList<Node> nodesToBeDeleted) {
        ...
        // A LoadIndexed on a local-memory HalfFloat[] (LocalArrayNode with CUDAKind.HALF) is
        // lowered to a ReadHalfFloatNode in CUDALoweringProvider#lowerLoadIndexedNode, so it is
        // a valid half-source for CUDAConvertHalfToFloat. Without this case, reading a HalfFloat
        ...
        if (phiNode.getStackKind().isObject()) {
            ...
            for (HalfFloatPlaceholder halfFloatPlaceholder : halfPhiNode.usages().filter(HalfFloatPlaceholder.class)) {
                for (FloatConvertNode floatConvertNode : halfFloatPlaceholder.usages().filter(FloatConvertNode.class)) {
                    CUDAConvertHalfToFloat oclConvertHalfToFloat = new CUDAConvertHalfToFloat(halfPhiNode);
    ...
    private static ValueNode getHalfOperand(ValueNode operand, StructuredGraph graph) {
        ...
        if (operand instanceof VectorLoadElementNode) {
            ...
            halfOperand = new VectorLoadElementNode(CUDAKind.HALF, loadElementNodeX.getVector(), loadElementNodeX.getLaneId());
    ...
    protected void run(StructuredGraph graph, TornadoHighTierContext context) {
        ...
        for (NewInstanceNode newInstanceNode : graph.getNodes().filter(NewInstanceNode.class)) {
            if (newInstanceNode.instanceClass().getAnnotation(HalfType.class) != null) {
                if (newInstanceNode.successors().first() instanceof NewHalfFloatInstance) {
                    ...
                    CUDAConvertFloatToHalf convertFloatToHalf = new CUDAConvertFloatToHalf(readValue);
                    graph.addWithoutUnique(convertFloatToHalf);
        ...
        for (LoadFieldNode loadFieldNode : graph.getNodes().filter(LoadFieldNode.class)) {
            ...
            if (loadFieldNode.usages().filter(CUDAConvertHalfToFloat.class).isNotEmpty()) {
        ...
        for (VectorValueNode vectorValueNode : graph.getNodes().filter(VectorValueNode.class)) {
            if (vectorValueNode.getCUDAKind().isHalf()) {
                for (Node vectorElement : vectorValueNode.inputs()) {
                    if (vectorElement instanceof VectorLoadElementNode vectorLoad) {
                        VectorLoadElementNode vectorLoadShort = new VectorLoadElementNode(CUDAKind.HALF, vectorLoad.getVector(), vectorLoad.getLaneId());
