...
import uk.ac.manchester.tornado.drivers.cuda.graal.nodes.NodeAtomic;
        ...
        if (!filter.isEmpty()) {
            for (NodeAtomic atomic : filter) {
                if (atomic.getAtomicNode() instanceof ParameterNode) {
                    ...
                    TornadoAtomicIntegerNode newNode = new TornadoAtomicIntegerNode(CUDAKind.INTEGER_ATOMIC_JAVA);
