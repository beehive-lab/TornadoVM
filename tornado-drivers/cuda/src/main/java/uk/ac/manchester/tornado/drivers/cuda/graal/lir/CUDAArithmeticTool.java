...
public class CUDAArithmeticTool extends ArithmeticLIRGenerator {
    ...
    public Value emitZeroExtend(Value value, int fromBits, int toBits, boolean requiresExplicitZeroExtend, boolean requiresLIRKindChange) {
        Logger.traceBuildLIR(Logger.BACKEND.CUDA, "emitZeroExtend: %s (from %d to %d, explicit=%b, kindChange=%b)",
                value, fromBits, toBits, requiresExplicitZeroExtend, requiresLIRKindChange);
        // Delegate to existing implementation - CUDA always needs explicit zero extend via masking
