...
import uk.ac.manchester.tornado.drivers.cuda.graal.lir.CUDAControlFlow.LoopPostOp;
...
public class CUDACompilationResultBuilder extends CompilationResultBuilder {
    ...
    /**
     * Checks if the {@link CUDANodeLIRBuilder(LoopBeginNode)} has been called right
     * before {@link CUDANodeLIRBuilder#emitIf}. In other words, that there is no
    ...
    public void emit(LIR lir) {
        ...
        new CUDAStructuredControlFlow(this).emit(cfg);
    ...
    public void finish() {
        ...
        if (source.contains("__nv_bfloat16") && !source.contains("cuda_bf16.h")) {
    ...
    void printBasicHIRBlockTrace(HIRBlock block) {
        if (isMergeHIRBlock(block)) {
            ...
            sb.append("]");
            ((CUDAAssembler) asm).emitLine("// BLOCK %d MERGES %s", block.getId(), sb.toString());
        } else {
            ((CUDAAssembler) asm).emitLine("// BLOCK %d", block.getId());
        }
    ...
    public TaskDataContext getTaskMetaData() {
        return ((CUDACompilationResult) compilationResult).getMeta();
    }
