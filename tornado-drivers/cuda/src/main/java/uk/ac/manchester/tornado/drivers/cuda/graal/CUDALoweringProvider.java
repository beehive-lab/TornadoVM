...
import uk.ac.manchester.tornado.drivers.cuda.graal.snippets.ReduceCPUSnippets;
import uk.ac.manchester.tornado.drivers.cuda.graal.snippets.ReduceGPUSnippets;
 ...
 * TornadoVM Mid-IR).
 */
public class CUDALoweringProvider extends DefaultJavaLoweringProvider {
    ...
    public CUDALoweringProvider(MetaAccessProvider metaAccess, ForeignCallsProvider foreignCalls, PlatformConfigurationProvider platformConfig, MetaAccessExtensionProvider metaAccessExtensionProvider,
            ConstantReflectionProvider constantReflection, TornadoVMConfigAccess vmConfig, CUDATargetDescription target) {
    ...
    /**
     * {@link CUDALoweringProvider#gpuSnippet} is set during the lowering phase.
    ...
    public boolean supportsBulkZeroing() {
        unimplemented("CUDALoweringProvider::supportsBulkZeroing unimplemented");
        return false;
    }
    ...
    public void lowerLoadIndexedNode(LoadIndexedNode loadIndexed, LoweringTool tool) {
        ...
        // An element read of an MMA accumulator fragment is a register access, not a memory
        // load: the fragment is a C local array in the generated source and has no address.
        // Supported: int32 fragments, constant index 0..3. Anything else on a fragment is refused
        // here rather than left for the address lowering to reject obscurely.
        if (isMMAFragmentValue(loadIndexed.array(), new java.util.HashSet<>())) {
            if (!isIntMMAFragment(loadIndexed.array(), new java.util.HashSet<>())) {
                throw new TornadoInternalError("reading an element of a float MMA fragment is not supported; only int32 accumulator fragments can be read in the kernel");
            }
            if (!(loadIndexed.index() instanceof ConstantNode constant) || constant.asJavaConstant() == null || !constant.asJavaConstant().getJavaKind().isNumericInteger()) {
                throw new TornadoInternalError("an MMA fragment element index must be a compile-time constant");
            }
            int index = constant.asJavaConstant().asInt();
            if (index < 0 || index > 3) {
                throw new TornadoInternalError("MMA fragment element index out of range: " + index + " (0..3)");
            }
            CUDAMMAFragmentElementReadNode read = graph.add(new CUDAMMAFragmentElementReadNode(loadIndexed.array(), index));
    ...
    protected void lowerLoadFieldNode(LoadFieldNode loadField, LoweringTool tool) {
        ...
        if (areCoopsEnabled && !field.getJavaKind().isPrimitive()) {
            CUDADecompressedReadFieldNode decompressedNode = graph.add(new CUDADecompressedReadFieldNode(object, address, loadStamp));
        ...
        if (invoke.callTarget() instanceof MethodCallTargetNode) {
            ...
            if (returnStamp instanceof ObjectStamp) {
                ...
                if (oclKind != CUDAKind.ILLEGAL) {
                    returnStampPair = StampPair.createSingle(CUDAStampFactory.getStampFor(oclKind));
                }
            }
    ...
    private void lowerLocalNewArray(StructuredGraph graph, int length, NewArrayNonVirtualizableNode newArray) {
        ...
        localArrayNode = graph.addWithoutUnique(new LocalArrayNode(CUDAArchitecture.localSpace, newArray.elementType(), newLengthNode));
    ...
    private AbstractWriteNode createMemWriteNode(JavaKind elementKind, ValueNode value, ValueNode array, AddressNode address, StructuredGraph graph, StoreIndexedNode storeIndexed) {
        ...
        if (!(valueStamp instanceof CUDAStamp) || !((CUDAStamp) valueStamp).getCUDAKind().isVector()) {
