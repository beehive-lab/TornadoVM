...
public class VectorAddNode extends BinaryNode implements LIRLowerable, VectorOp {
    ...
    public VectorAddNode(CUDAKind kind, ValueNode x, ValueNode y) {
        super(TYPE, CUDAStampFactory.getStampFor(kind), x, y);
    }

    @Override
    public Stamp foldStamp(Stamp stampX, Stamp stampY) {
        Stamp currentStamp = stamp(NodeView.DEFAULT);
        if (currentStamp instanceof CUDAStamp) {
            return currentStamp;
        }
        return (stampX instanceof CUDAStamp) ? stampX.join(stampY) : stampY.join(stampX);
    }
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        gen.getLIRGeneratorTool().append(new AssignStmt(result, new CUDABinary.Expr(CUDABinaryOp.ADD, gen.getLIRGeneratorTool().getLIRKind(stamp), input1, input2)));
