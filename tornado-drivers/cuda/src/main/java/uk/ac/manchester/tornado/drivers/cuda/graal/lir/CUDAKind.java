...
public enum CUDAKind implements PlatformKind {
    ...
    private final CUDAKind elementKind;
    private final Class<?> javaClass;
    @SuppressWarnings({ "unchecked", "rawtypes" })
    private final EnumKey key = new EnumKey(this);

    CUDAKind(int size, Class<?> javaClass) {
        this(size, javaClass, null);
    }

    CUDAKind(int size, Class<?> javaClass, CUDAKind kind) {
        this.kind = this;
    ...
    public static CUDAKind fromResolvedJavaType(ResolvedJavaType type) {
        if (!type.isArray()) {
            for (CUDAKind k : CUDAKind.values()) {
    ...
    public static CUDAKind fromResolvedJavaKind(JavaKind javaKind) {
        for (CUDAKind k : CUDAKind.values()) {
    ...
    public static CUDAAssembler.CUDABinaryTemplate resolvePrivateTemplateType(JavaKind type) {
        if (type == JavaKind.Int) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_PRIVATE_INT_ARRAY;
        } else if (type == JavaKind.Double) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_PRIVATE_DOUBLE_ARRAY;
        } else if (type == JavaKind.Float) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_PRIVATE_FLOAT_ARRAY;
        } else if (type == JavaKind.Short) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_PRIVATE_SHORT_ARRAY;
        } else if (type == JavaKind.Long) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_PRIVATE_LONG_ARRAY;
        } else if (type == JavaKind.Char) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_PRIVATE_CHAR_ARRAY;
        } else if (type == JavaKind.Byte) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_PRIVATE_BYTE_ARRAY;
        }
        return null;
    }

    public static CUDAAssembler.CUDABinaryTemplate resolvePrivatePointerTemplate(JavaKind type) {
        return switch (type) {
            case Int -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_INT_ARRAY_PTR;
            case Double -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_DOUBLE_ARRAY_PTR;
            case Float -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_FLOAT_ARRAY_PTR;
            case Short -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_SHORT_ARRAY_PTR;
            case Long -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_LONG_ARRAY_PTR;
            case Char -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_CHAR_ARRAY_PTR;
            case Byte -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_BYTE_ARRAY_PTR;
            default -> null;
        };
    }

    public static CUDAAssembler.CUDABinaryTemplate resolvePrivatePointerCopyTemplate(JavaKind type) {
        return switch (type) {
            case Int -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_INT_ARRAY_PTR_COPY;
            case Double -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_DOUBLE_ARRAY_PTR_COPY;
            case Float -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_FLOAT_ARRAY_PTR_COPY;
            case Short -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_SHORT_ARRAY_PTR_COPY;
            case Long -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_LONG_ARRAY_PTR_COPY;
            case Char -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_CHAR_ARRAY_PTR_COPY;
            case Byte -> CUDAAssembler.CUDABinaryTemplate.PRIVATE_BYTE_ARRAY_PTR_COPY;
            default -> null;
        };
    }
    ...
    public static CUDAAssembler.CUDABinaryTemplate resolveTemplateType(JavaKind type, CUDAKind kind) {
        if (type == JavaKind.Int) {
            ...
            return CUDAAssembler.CUDABinaryTemplate.NEW_LOCAL_DOUBLE_ARRAY;
        } else if (type == JavaKind.Float) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_LOCAL_FLOAT_ARRAY;
        } else if (type == JavaKind.Short) {
            if (kind == CUDAKind.HALF) {
                return CUDAAssembler.CUDABinaryTemplate.NEW_LOCAL_HALF_FLOAT_ARRAY;
            } else {
                return CUDAAssembler.CUDABinaryTemplate.NEW_LOCAL_SHORT_ARRAY;
            }
        } else if (type == JavaKind.Long) {
            return CUDAAssembler.CUDABinaryTemplate.NEW_LOCAL_LONG_ARRAY;
        } else if (type == JavaKind.Char || type == JavaKind.Byte) {
            // In CUDADriver bytes are treated as chars
            return CUDAAssembler.CUDABinaryTemplate.NEW_LOCAL_CHAR_ARRAY;
        }
        return null;
    }

    public static CUDAAssembler.CUDABinaryTemplate resolveTemplateType(ResolvedJavaType type, CUDAKind kind) {
        return resolveTemplateType(type.getJavaKind(), kind);
    }

    public static CUDAAssembler.CUDABinaryTemplate resolvePrivateTemplateType(ResolvedJavaType type) {
        return resolvePrivateTemplateType(type.getJavaKind());
    }

    public static CUDAAssembler.CUDABinaryTemplate resolvePrivatePointerTemplate(ResolvedJavaType type) {
        return resolvePrivatePointerTemplate(type.getJavaKind());
    }

    public static CUDAAssembler.CUDABinaryTemplate resolvePrivatePointerCopyTemplate(ResolvedJavaType type) {
        return resolvePrivatePointerCopyTemplate(type.getJavaKind());
    }

    public static CUDAKind resolveToVectorKind(ResolvedJavaType type) {
        if (!type.isPrimitive() && type.getAnnotation(Vector.class) != null) {
            ...
            return CUDAKind.valueOf(simpleName);
        }
        return CUDAKind.ILLEGAL;
    }

    public static int lookupTypeIndex(CUDAKind kind) {
        return switch (kind) {
            case SHORT -> 0;
            case INT -> 1;
            case FLOAT -> 2;
    ...
    public String toString() {
        if (this == CUDAKind.ATOMIC_ADD_INT) {
            return "int";
        } else if (this == CUDAKind.ATOMIC_SUB_INT) {
            return "int";
        } else if (this == CUDAKind.ATOMIC_MUL_INT) {
            return "int";
        } else if (this == CUDAKind.ATOMIC_ADD_LONG) {
            return "long";
