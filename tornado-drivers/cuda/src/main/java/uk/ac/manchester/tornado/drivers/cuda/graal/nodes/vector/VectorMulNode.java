...
public class VectorMulNode extends FloatingNode implements LIRLowerable, VectorOp {
    ...
    public VectorMulNode(CUDAKind kind, ValueNode x, ValueNode y) {
        this(TYPE, kind, x, y);
    }

    protected VectorMulNode(NodeClass<? extends VectorMulNode> c, CUDAKind kind, ValueNode x, ValueNode y) {
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        gen.getLIRGeneratorTool().append(new AssignStmt(result, new CUDABinary.Expr(CUDABinaryOp.MUL, gen.getLIRGeneratorTool().getLIRKind(stamp), input1, input2)));
