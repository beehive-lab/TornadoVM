...
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.ATAN2;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.CROSS;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.DOT;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.FLOAT_MAX;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.FLOAT_MIN;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.FLOAT_POW;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.INT_MAX;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDABinaryIntrinsic.INT_MIN;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDATernaryIntrinsic.CLAMP;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.ABS;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.ACOS;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.ACOSH;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.ASIN;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.ASINH;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.ATAN;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.CEIL;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.COS;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.COSPI;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.EXP;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.FLOAT_ABS;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.FLOAT_FLOOR;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.FLOAT_TRUNC;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.LOG;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.NATIVE_COS;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.NATIVE_SIN;
...
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.SINPI;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.SQRT;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.TAN;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssembler.CUDAUnaryIntrinsic.TANH;
...
public class CUDABuiltinTool {
    ...
    public Value genFloatASin(Value input) {
        ...
        return new CUDAUnary.Intrinsic(ASIN, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatASinh(Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genAsinh: asinh(%s)", input);
        return new CUDAUnary.Intrinsic(ASINH, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatATan(Value input) {
        ...
        return new CUDAUnary.Intrinsic(ATAN, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatCeil(Value input) {
        ...
        return new CUDAUnary.Intrinsic(CEIL, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatCos(Value input) {
        ...
        if (TornadoOptions.ENABLE_NATIVE_FUNCTION) {
            return new CUDAUnary.Intrinsic(NATIVE_COS, LIRKind.value(input.getPlatformKind()), input);
        }
        return new CUDAUnary.Intrinsic(COS, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatExp(Value input) {
        ...
        return new CUDAUnary.Intrinsic(EXP, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatFloor(Value input) {
        ...
        return new CUDAUnary.Intrinsic(FLOAT_FLOOR, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatLog(Value input) {
        ...
        return new CUDAUnary.Intrinsic(LOG, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatRadians(Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genFloatRadians: radians(%s)", input);
        return new CUDAUnary.Intrinsic(RADIANS, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatCosPI(Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genFloatCosPI: cospi(%s)", input);
        return new CUDAUnary.Intrinsic(COSPI, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatSinPI(Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genFloatCosPI: sinpi(%s)", input);
        return new CUDAUnary.Intrinsic(SINPI, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatSign(Value input) {
        ...
        return new CUDAUnary.Intrinsic(SIGN, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatSin(Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genSin: sin(%s)", input);
        if (TornadoOptions.ENABLE_NATIVE_FUNCTION) {
            return new CUDAUnary.Intrinsic(NATIVE_SIN, LIRKind.value(input.getPlatformKind()), input);
        }
        return new CUDAUnary.Intrinsic(SIN, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatTan(Value input) {
        ...
        if (TornadoOptions.ENABLE_NATIVE_FUNCTION) {
            return new CUDAUnary.Intrinsic(NATIVE_TAN, LIRKind.value(input.getPlatformKind()), input);
        }
        return new CUDAUnary.Intrinsic(TAN, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatTanh(Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genTanh: tanh(%s)", input);
        return new CUDAUnary.Intrinsic(TANH, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genFloatTrunc(Value input) {
        ...
        return new CUDAUnary.Intrinsic(FLOAT_TRUNC, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatATan2(Value x, Value y) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genFloatAtan2: atan(%s, %s)", x, y);
        return new CUDABinary.Intrinsic(ATAN2, LIRKind.combine(x, y), x, y);
    }
    ...
    public Value genFloatMax(Value x, Value y) {
        ...
        return new CUDABinary.Intrinsic(FLOAT_MAX, LIRKind.combine(x, y), x, y);
    }

    public Value genFloatMin(Value x, Value y) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genFloatMin: min(%s,%s)", x, y);
        return new CUDABinary.Intrinsic(FLOAT_MIN, LIRKind.combine(x, y), x, y);
    }
    ...
    public Value genFloatPow(Value x, Value y) {
        ...
        return new CUDABinary.Intrinsic(FLOAT_POW, LIRKind.combine(x, y), x, y);
    }
    ...
    public Value genIntAbs(Value input) {
        ...
        return new CUDAUnary.Intrinsic(ABS, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genFloatSqrt(Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genAbs: sqrt(%s)", input);
        if (TornadoOptions.ENABLE_NATIVE_FUNCTION) {
            return new CUDAUnary.Intrinsic(NATIVE_SQRT, LIRKind.value(input.getPlatformKind()), input);
        }
        return new CUDAUnary.Intrinsic(SQRT, LIRKind.value(input.getPlatformKind()), input);
    }

    public Value genIntMax(Value x, Value y) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genMax: max(%s,%s)", x, y);
        return new CUDABinary.Intrinsic(INT_MAX, LIRKind.combine(x, y), x, y);
    }

    public Value genIntMin(Value x, Value y) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genMin: min(%s,%s)", x, y);
        return new CUDABinary.Intrinsic(INT_MIN, LIRKind.combine(x, y), x, y);
    }
    ...
    public Value genIntClamp(Value x, Value y, Value z) {
        ...
        return new CUDATernary.Intrinsic(CLAMP, LIRKind.combine(x, y, z), x, y, z);
    }
    ...
    public Value genFloatAbs(Value input) {
        ...
        return new CUDAUnary.Intrinsic(FLOAT_ABS, LIRKind.value(input.getPlatformKind()), input);
    }
    ...
    public Value genGeometricDot(Value x, Value y) {
        ...
        return new CUDABinary.Intrinsic(DOT, LIRKind.combine(x, y), x, y);
    }

    public Value genGeometricCross(Value x, Value y) {
        Logger.traceBuildLIR(Logger.BACKEND.OpenCL, "genCross: cross(%s,%s)", x, y);
        return new CUDABinary.Intrinsic(CROSS, LIRKind.combine(x, y), x, y);
    }
