...
/**
 * Test the CUDADriver JIT Compiler and connection with the Tornado Runtime
 * Environment.
 *
 */
public class TestCUDAJITCompiler {
    ...
    public static void main(String[] args) {
        System.out.print("Running Native: uk.ac.manchester.tornado.drivers.cuda.tests.TestCUDAJITCompiler");
        new TestCUDAJITCompiler().test();
    }

    public MetaCompilation compileMethod(long executionPlanId, Class<?> klass, String methodName, CUDATornadoDevice tornadoDevice, Object... parameters) {
        ...
        CUDABackend openCLBackend = tornadoRuntime.getBackend(CUDABackendImpl.class).getDefaultBackend();

        // Get the default CUDADriver device
        ...
        CUDACompilationResult compilationResult = CUDACompiler.compileSketchForDevice(sketch, compilableTask, (CUDAProviders) providers, openCLBackend, new EmptyProfiler());

        // Install the CUDADriver Code in the VM
        CUDAInstalledCode openCLCode = tornadoDevice.getDeviceContext().installCode(executionPlanId, compilationResult);

        return new MetaCompilation(taskMeta, openCLCode);
    }

    public void runWithOpenCLAPI(Long executionPlanId, CUDATornadoDevice tornadoDevice, CUDAInstalledCode openCLCode, TaskDataContext taskMeta, int[] a, int[] b, float[] c) {
        CUDADriver.run(executionPlanId, tornadoDevice, openCLCode, taskMeta, new Access[] { Access.READ_ONLY, Access.READ_ONLY, Access.WRITE_ONLY }, a, b, c);
    }

    public void run(CUDATornadoDevice tornadoDevice, CUDAInstalledCode openCLCode, TaskDataContext taskMeta, int[] a, int[] b, float[] c) {
    ...
    public void test() {
        ...
        CUDATornadoDevice tornadoDevice = (CUDATornadoDevice) TornadoCoreRuntime.getTornadoRuntime().getBackend(CUDABackendImpl.class).getDefaultDevice();

        MetaCompilation compileMethod = compileMethod(executionPlanId, TestCUDAJITCompiler.class, "methodToCompile", tornadoDevice, a, b, c);

        // Check with all internal APIs
        run(tornadoDevice, (CUDAInstalledCode) compileMethod.getInstalledCode(), compileMethod.getTaskMeta(), a, b, c);


        // Check with CUDADriver API
        runWithOpenCLAPI(executionPlanId, tornadoDevice, (CUDAInstalledCode) compileMethod.getInstalledCode(), compileMethod.getTaskMeta(), a, b, c);
