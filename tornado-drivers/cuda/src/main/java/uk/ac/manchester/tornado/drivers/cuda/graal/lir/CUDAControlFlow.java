...
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssemblerConstants.COLON;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssemblerConstants.DEFAULT_CASE;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssemblerConstants.FOR_LOOP;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssemblerConstants.IF_STMT;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssemblerConstants.NOT;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssemblerConstants.OPEN_PARENTHESIS;
import static uk.ac.manchester.tornado.drivers.cuda.graal.asm.CUDAAssemblerConstants.SWITCH;

/**
 * CUDADriver Code Generation for all control-flow constructs.
 */
public class CUDAControlFlow {
    ...
    public static class LoopConditionOp extends AbstractInstruction {
        ...
        public void emitCode(CUDACompilationResultBuilder crb, CUDAAssembler asm) {
            ...
            if (condition instanceof CUDALIROp) {
                ((CUDALIROp) condition).emit(crb, asm);
            } else {
                asm.emitValue(crb, condition);
            }

            if (condition.getPlatformKind() == CUDAKind.INT) {
                asm.emit(" == 1");
            }
