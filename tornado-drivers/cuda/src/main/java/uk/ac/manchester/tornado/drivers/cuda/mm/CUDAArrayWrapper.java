...
public abstract class CUDAArrayWrapper<T> implements XPUBuffer {
    ...
    protected CUDAArrayWrapper(final CUDADeviceContext device, final JavaKind kind, long batchSize, Access access) {
    ...
    protected CUDAArrayWrapper(final T array, final CUDADeviceContext device, final JavaKind kind, long batchSize, Access access) {
     ...
     */
    private CUDAByteBuffer buildArrayHeader(final int arraySize) {
        final CUDAByteBuffer header = getArrayHeader();
    ...
    private CUDAByteBuffer buildArrayHeaderBatch(final long arraySize) {
        final CUDAByteBuffer header = getArrayHeader();
        for (int i = 0; i < arrayLengthOffset; i++) {
            header.buffer.put((byte) 0);
        }
    ...
    private CUDAByteBuffer getArrayHeader() {
        final CUDAByteBuffer header = new CUDAByteBuffer(deviceContext, bufferId, bufferOffset, arrayHeaderSize);
        header.buffer.clear();
        return header;
    }
