...
public class VectorSubNode extends FloatingNode implements LIRLowerable, VectorOp {
    ...
    public VectorSubNode(CUDAKind kind, ValueNode x, ValueNode y) {
        this(TYPE, kind, x, y);
    }

    protected VectorSubNode(NodeClass<? extends VectorSubNode> c, CUDAKind kind, ValueNode x, ValueNode y) {
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        gen.getLIRGeneratorTool().append(new AssignStmt(result, new CUDABinary.Expr(CUDABinaryOp.SUB, gen.getLIRGeneratorTool().getLIRKind(stamp), input1, input2)));
