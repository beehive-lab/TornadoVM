...
import uk.ac.manchester.tornado.drivers.cuda.graal.lir.CUDAVectorAssign;
...
public class VectorValueNode extends FloatingNode implements LIRLowerable {
    ...
    public VectorValueNode(CUDAKind kind) {
        super(TYPE, CUDAStampFactory.getStampFor(kind));
        this.kind = kind;
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        if (origin instanceof InvokeNode) {
            ...
            final Value phiOperand = ((CUDANodeLIRBuilder) gen).operandForPhi(phi);

            final AllocatableValue result = (gen.hasOperand(this)) ? (Variable) gen.operand(this) : tool.newVariable(LIRKind.value(getCUDAKind()));
            tool.append(new CUDALIRStmt.AssignStmt(result, phiOperand));
            gen.setResult(this, result);

        } else if (origin instanceof ParameterNode) {
            gen.setResult(this, gen.operand(origin));
        } else if (origin == null) {
            final AllocatableValue result = tool.newVariable(LIRKind.value(getCUDAKind()));
            ...
            if (firstValue instanceof VectorValueNode || firstValue instanceof VectorOp) {
                tool.append(new CUDALIRStmt.AssignStmt(result, gen.operand(values.first())));
                gen.setResult(this, result);
    ...
    private void generateVectorAssign(NodeLIRBuilderTool gen, LIRGeneratorTool tool, AllocatableValue result) {
        ...
        if (kind.getVectorLength() >= 2) {
            if (((CUDAKind) s0.getPlatformKind()).isVector()) {
        ...
        switch (kind.getVectorLength()) {
            case 2: {
                final CUDAOp2 op2 = VectorUtil.resolveAssignOp2(getCUDAKind());
                s1 = getParam(gen, tool, 1);
                assignExpr = new CUDAVectorAssign.Assign2Expr(op2, getCUDAKind(), s0, s1);
                break;
            }
            case 3: {
                final CUDAOp3 op3 = VectorUtil.resolveAssignOp3(getCUDAKind());
                s1 = getParam(gen, tool, 1);
                s2 = getParam(gen, tool, 2);
                assignExpr = new CUDAVectorAssign.Assign3Expr(op3, getCUDAKind(), s0, s1, s2);
                break;
            }
            case 4: {
                final CUDAOp4 op4 = VectorUtil.resolveAssignOp4(getCUDAKind());
                s1 = getParam(gen, tool, 1);
                s2 = getParam(gen, tool, 2);
                s3 = getParam(gen, tool, 3);
                assignExpr = new CUDAVectorAssign.Assign4Expr(op4, getCUDAKind(), s0, s1, s2, s3);
                break;
            }
            case 8: {
                final CUDAOp8 op8 = VectorUtil.resolveAssignOp8(getCUDAKind());
                s1 = getParam(gen, tool, 1);
                s2 = getParam(gen, tool, 2);
                s3 = getParam(gen, tool, 3);
                ...
                assignExpr = new CUDAVectorAssign.Assign8Expr(op8, getCUDAKind(), s0, s1, s2, s3, s4, s5, s6, s7);
                break;

            }
            case 16: {
                final CUDAAssembler.CUDAOp16 op16 = VectorUtil.resolveAssignOp16(getCUDAKind());
                s1 = getParam(gen, tool, 1);
                s2 = getParam(gen, tool, 2);
                s3 = getParam(gen, tool, 3);
                s4 = getParam(gen, tool, 4);
                ...
                assignExpr = new CUDAVectorAssign.Assign16Expr(op16, getCUDAKind(), s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15);
                break;
            }
            default:
        ...
        tool.append(new CUDALIRStmt.AssignStmt(result, assignExpr));
