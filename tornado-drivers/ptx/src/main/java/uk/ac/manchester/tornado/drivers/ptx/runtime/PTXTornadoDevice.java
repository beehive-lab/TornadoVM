...
public class PTXTornadoDevice implements TornadoXPUDevice {
    ...
    private TornadoInstalledCode compileTask(long executionPlanId, SchedulableTask task) {
        ...
        try {
            ...
            TornadoInstalledCode installedCode = deviceContext.installCode(executionPlanId, taskMeta, result, resolvedMethod.getName());
    ...
    private TornadoInstalledCode compilePreBuiltTask(long executionPlanId, SchedulableTask task) {
        ...
        try {
            ...
            return deviceContext.installCode(executionPlanId, executable.meta(), functionName, source, executable.getEntryPoint(), task.meta().isPrintKernelEnabled());
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
