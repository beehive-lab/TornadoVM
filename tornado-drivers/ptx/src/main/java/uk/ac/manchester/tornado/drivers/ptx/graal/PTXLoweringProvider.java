...
/**
 * Lower IR from one representation to another (e.g., from TornadoVM High-IR to TornadoVM Mid-IR).
 */
public class PTXLoweringProvider extends DefaultJavaLoweringProvider {
    ...
    /**
     * {@link PTXLoweringProvider#gpuSnippet} is set during the lowering phase. Therefore, this method must be called after a lowering phase in order to get the correct result.
     *
     * @return boolean
     */
    ...
    private void lowerInvoke(Invoke invoke, LoweringTool tool, StructuredGraph graph) {
        if (invoke.callTarget() instanceof MethodCallTargetNode) {
            ...
            LoweredCallTargetNode loweredCallTarget = graph.add(
                    new TornadoDirectCallTargetNode(parameters.toArray(new ValueNode[parameters.size()]), returnStampPair, signature, callTarget.targetMethod(), HotSpotCallingConventionType.JavaCall,
