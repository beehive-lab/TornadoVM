...
public abstract class SPIRVDeviceContext implements TornadoDeviceContext {
    ...
    @Override
    public boolean isFP64Supported() { return device.isDeviceDoubleFPSupported(); }
    ...
    public int readBuffer(long bufferId, long offset, long bytes, long hostpointer, long hostOffset, int[] waitEvents) {
        ProfilerTransfer profilerTransfer = createStartAndStopBufferTimers();
        spirvContext.readBuffer(getDeviceIndex(), bufferId, offset, bytes, hostpointer, hostOffset, waitEvents, profilerTransfer);
        return spirvEventPool.registerEvent(EventDescriptor.DESC_READ_SEGMENT, profilerTransfer);
    }
    ...
    public int enqueueWriteBuffer(long bufferId, long offset, long bytes, long hostpointer, long hostOffset, int[] waitEvents) {
        ProfilerTransfer profilerTransfer = createStartAndStopBufferTimers();
        spirvContext.enqueueWriteBuffer(device.getDeviceIndex(), bufferId, offset, bytes, hostpointer, hostOffset, waitEvents, profilerTransfer);
        return spirvEventPool.registerEvent(EventDescriptor.DESC_WRITE_SEGMENT, profilerTransfer);
    }
