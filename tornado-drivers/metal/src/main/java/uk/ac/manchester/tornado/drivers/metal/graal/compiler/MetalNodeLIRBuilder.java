...
import uk.ac.manchester.tornado.drivers.metal.graal.asm.MetalAssembler.MetalBinaryIntrinsicCmp;
...
import uk.ac.manchester.tornado.drivers.metal.graal.lir.MetalDirectCall;
...
public class MetalNodeLIRBuilder extends NodeLIRBuilder {

    private boolean elseClause;

    public MetalNodeLIRBuilder(final StructuredGraph graph, final LIRGeneratorTool gen, NodeMatchRules nodeMatchRules) {
    ...
    public void doBlock(final HIRBlock block, final StructuredGraph graph, final BlockMap<List<Node>> blockMap, boolean isKernel) {
        ...
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "%s - block %s", graph.method().getName(), block);
    ...
    private void platformPatch(boolean isKernel) {
        ...
        if (op instanceof ExprStmt exprStmtOp) {
            ...
            if (expr.getExpr() instanceof MetalUnary.Expr && ((MetalUnary.Expr) expr.getExpr()).getOpcode().equals(MetalUnaryOp.RETURN)) {
                MetalUnary.Expr returnExpr = (MetalUnary.Expr) expr.getExpr();
                append(new ExprStmt(new MetalNullary.Expr(MetalNullaryOp.RETURN, LIRKind.value(MetalKind.ILLEGAL))));
                insns.remove(index);
                LIRKind lirKind = LIRKind.value(returnExpr.getPlatformKind());
                final AllocatableValue slotAddress = new MetalReturnSlot(lirKind);
    ...
    private Value emitNegatedLogicNode(final LogicNode node) {
        Value result;
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitNegatedLogicNode: %s", node);
        LIRKind intLirKind = LIRKind.value(MetalKind.INT);
        LIRKind boolLirKind = LIRKind.value(MetalKind.BOOL);
        if (node instanceof LogicalEqualsNode logicalEqualsNode) {
            ...
            result = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_NE, boolLirKind, x, y);
            ...
            Value cond1 = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_LT, boolLirKind, x, gen.emitConstant(intLirKind, JavaConstant.forInt(0)));
            Value cond2 = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_GTE, boolLirKind, x, y);
            result = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.LOGICAL_OR, boolLirKind, cond1, cond2);
            ...
            result = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_GTE, boolLirKind, x, y);
    ...
    private MetalLIROp emitLogicNode(final LogicNode node) {
        Value result;
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitLogicNode: %s", node);
        LIRKind intLirKind = LIRKind.value(MetalKind.INT);
        LIRKind boolLirKind = LIRKind.value(MetalKind.BOOL);
        if (node instanceof LogicalEqualsNode logicalEqualsNode) {
            final Value x = operandOrConjunction(logicalEqualsNode.getX());
            final Value y = operandOrConjunction(logicalEqualsNode.getY());
            result = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_EQ, boolLirKind, x, y);
            ...
            result = getGen().getArithmetic().genUnaryExpr(MetalUnaryOp.LOGICAL_NOT, boolLirKind, value);
        } else if (node instanceof FloatEqualsNode floatEqualsNode) {
            final Value x = operand(floatEqualsNode.getX());
            final Value y = operand(floatEqualsNode.getY());
            ...
            final Value x = operand(integerBelowNode.getX());
            final Value y = operand(integerBelowNode.getY());
            Value cond1 = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_GTE, boolLirKind, x, gen.emitConstant(intLirKind, JavaConstant.forInt(0)));
            Value cond2 = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_LT, boolLirKind, x, y);
            result = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.LOGICAL_AND, boolLirKind, cond1, cond2);
        } else if (node instanceof IntegerEqualsNode integerEqualsNode) {
            final Value x = operand(integerEqualsNode.getX());
            final Value y = operand(integerEqualsNode.getY());
            result = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_EQ, boolLirKind, x, y);
            ...
            final Value x = operand(integerLessThanNode.getX());
            final Value y = operand(integerLessThanNode.getY());
            result = getGen().getArithmetic().genBinaryExpr(MetalBinaryOp.RELATIONAL_LT, boolLirKind, x, y);
        } else if (node instanceof IsNullNode isNullNode) {
            final Value value = operand(isNullNode.getValue());
    ...
    protected void emitDirectCall(final DirectCallTargetNode callTarget, final Value result, final Value[] parameters, final Value[] temps, final LIRFrameState callState) {
        final MetalDirectCall call = new MetalDirectCall(callTarget, result, parameters, callState);
        if (isLegal(result)) {
            append(new MetalLIRStmt.AssignStmt(gen.asAllocatable(result), call));
        } else {
            append(new MetalLIRStmt.ExprStmt(call));
        }
    }
    ...
    public void emitIf(final IfNode x) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitIf: %s, condition=%s\n", x, x.condition().getClass().getName());
        ...
        if (falseBranch.getTargetBlock().isExceptionEntry()) {
            Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitExceptionEntry");
        ...
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "condition: %s -> %s", x.condition(), condition);

        if (isLoop) {
            // HERE NEED TO ADD THE PRAGMA UNROLL
            append(new MetalControlFlow.LoopConditionOp(condition));
        } else if (elseClause) {
    ...
    private void emitLoopBegin(final LoopBeginNode loopBeginNode) {
        ...
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "visiting emitLoopBegin %s", loopBeginNode);
    ...
    public void visitLoopEnd(final LoopEndNode loopEnd) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "visiting LoopEndNode: %s", loopEnd);
        ...
        for (ValuePhiNode phi : phis) {
            ...
            if (!dest.equals(src)) {
                append(new MetalLIRStmt.AssignStmt(dest, src));
            }
        }
    }
    ...
    public void visitMerge(final AbstractMergeNode mergeNode) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "visitMerge: ", mergeNode);
    ...
    protected void emitNode(final ValueNode node) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitNode: %s", node);
    ...
    public void emitSwitch(SwitchNode x) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "SWITCH NODE Metal:");
    ...
    private void emitShortCircuitOrNode(ShortCircuitOrNode node) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitShortCircuitOrNode: %s, (X: %s - isNegated: %s) || (Y: %s - isNegated: %s)", node, node.getX(), node.isXNegated(), node.getY(), node
                .isYNegated());

        LIRKind lirKind = LIRKind.value(MetalKind.BOOL);
        ...
        Value orExpr = new MetalBinary.Expr(MetalBinaryOp.LOGICAL_OR, lirKind, x, y);
    ...
    public void emitConditionalNode(ConditionalNode conditional) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitConditionalNode: %s", conditional);
    ...
    private void emitLoopExit(LoopExitNode node) {
        if (!node.loopBegin().getBlockNodes().contains((FixedNode) node.predecessor())) {
            append(new MetalControlFlow.LoopBreakOp());
        }
    }
    ...
    private MetalLIRGenerator getGen() {
        return (MetalLIRGenerator) gen;
    }
    ...
    public void visitEndNode(final AbstractEndNode end) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "visitEnd: %s", end);
        ...
        if (shouldRelocateInstructions) {
            append(new MetalLIRStmt.MarkRelocateInstruction());
        }
        ...
        for (ValuePhiNode phi : merge.valuePhis()) {
            ...
            if (!phi.isLoopPhi() && phi.singleValueOrThis() == phi || (value instanceof PhiNode && !((PhiNode) value).isLoopPhi())) {
                ...
                if (((!phi.isLoopPhi()) && (phi.singleValueOrThis() == phi)) || (value instanceof PhiNode && !((PhiNode) value).isLoopPhi())) {
                    ...
                    final AllocatableValue result = gen.asAllocatable(operandForPhi(phi));
                    append(new MetalLIRStmt.AssignStmt(result, operand(value)));
                }
            }
        }
    }
