...
import uk.ac.manchester.tornado.drivers.metal.graal.phases.InfinityReplacementPhase;
import uk.ac.manchester.tornado.drivers.metal.graal.phases.InverseSquareRootPhase;
...
import uk.ac.manchester.tornado.drivers.metal.graal.phases.TornadoHalfFloatVectorOffset;
import uk.ac.manchester.tornado.runtime.common.TornadoOptions;
import uk.ac.manchester.tornado.runtime.graal.compiler.TornadoLowTier;

public class MetalLowTier extends TornadoLowTier {

    TornadoDeviceContext tornadoDeviceContext;

    public MetalLowTier(OptionValues options, TornadoDeviceContext tornadoDeviceContext, AddressLowering addressLowering) {
        this.tornadoDeviceContext = tornadoDeviceContext;
        CanonicalizerPhase canonicalizer = getCannonicalizer(options);

        appendPhase(new MetalFP64SupportPhase(tornadoDeviceContext));

        appendPhase(new MetalFP16SupportPhase(tornadoDeviceContext));
