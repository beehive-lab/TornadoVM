...
public class MetalCommandQueue extends CommandQueue {
    ...
    private final int metalVersion;

    public MetalCommandQueue(long commandQueuePtr, long properties, int version) {
        this.commandQueuePtr = commandQueuePtr;
        this.properties = properties;
        this.buffer = ByteBuffer.allocate(128);
        this.buffer.order(Metal.BYTE_ORDER);
        this.metalVersion = version;
    }
     ...
     *
     * @param queueId
     *     Metal command queue object
     * @param kernelId
     *     Metal kernel ID object
     * @param dim
    ...
    public long enqueueBarrier(long[] waitEvents) {
        return (metalVersion < 120) ? enqueueBarrier_Metalv1_1(waitEvents) : enqueueBarrier_Metalv1_2(waitEvents);
    }
    ...
    public long enqueueMarker(long[] waitEvents) {
        return (metalVersion < 120) ? enqueueMarker11(waitEvents) : enqueueMarker12(waitEvents);
    }

    private int enqueueMarker11(long[] events) {
        return enqueueBarrier_Metalv1_1(events);
    }
