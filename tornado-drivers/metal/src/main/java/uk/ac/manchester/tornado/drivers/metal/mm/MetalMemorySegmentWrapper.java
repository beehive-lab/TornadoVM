...
public class MetalMemorySegmentWrapper implements XPUBuffer {
    ...
    public MetalMemorySegmentWrapper(long bufferSize, MetalDeviceContext deviceContext, long batchSize, Access access, int sizeOfType) {
    ...
    public MetalMemorySegmentWrapper(MetalDeviceContext deviceContext, long batchSize, Access access, int sizeOfType) {
    ...
    public void mapOnDeviceMemoryRegion(long executionPlanId, XPUBuffer srcPointer, long offset) {
        if (!(srcPointer instanceof MetalMemorySegmentWrapper oclMemorySegmentWrapper)) {
            throw new TornadoRuntimeException("[ERROR] copy pointer must be an instance of MetalMemorySegmentWrapper: " + srcPointer);
        }
