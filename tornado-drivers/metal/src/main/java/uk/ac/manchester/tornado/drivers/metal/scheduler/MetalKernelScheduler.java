...
public abstract class MetalKernelScheduler {
    ...
    protected MetalKernelScheduler(final MetalDeviceContext context) {
    ...
    public int submit(long executionPlanId, final MetalKernel kernel, final TaskDataContext meta, long batchThreads) {
        return submit(executionPlanId, kernel, meta, (long[]) null, batchThreads);
    }
    ...
    public int launch(long executionPlanId, final MetalKernel kernel, final TaskDataContext meta, final int[] waitEvents, long batchThreads) {
        long[] waitEventsLong = null;
        if (waitEvents != null) {
            waitEventsLong = new long[waitEvents.length];
            for (int i = 0; i < waitEvents.length; i++) {
                waitEventsLong[i] = waitEvents[i];
            }
        }
        return launch(executionPlanId, kernel, meta, waitEventsLong, batchThreads);
    }
    ...
    public int launch(long executionPlanId, final MetalKernel kernel, final TaskDataContext meta, final long[] waitEvents, long batchThreads) {
        if (meta.isWorkerGridAvailable()) {
            ...
            return deviceContext.enqueueNDRangeKernel(executionPlanId, kernel, meta.getDims(), meta.getGlobalOffset(), meta.getGlobalWork(), (meta.shouldUseMetalDriverScheduling()
                    ? null
     ...
     * not fit, it sets the local work group to null, so the Metal driver chooses a
     * default value. In this case, the threads configured in the local work sizes
     * depends on each Metal driver.
     *
     * @param meta
     *     TaskMetaData.
     */
