...
public abstract class MetalLIROp extends Value {

    public MetalLIROp(LIRKind lirKind) {
        super(lirKind);
    }

    public final void emit(MetalCompilationResultBuilder crb) {
        emit(crb, crb.getAssembler());
    }

    public abstract void emit(MetalCompilationResultBuilder crb, MetalAssembler asm);

    public LIRKind getLIRKind() {
        return (LIRKind) this.getValueKind();
    }

    public MetalKind getMetalPlatformKind() {
        PlatformKind platformKind = getPlatformKind();
        return (platformKind instanceof MetalKind) ? (MetalKind) platformKind : MetalKind.ILLEGAL;
    }
