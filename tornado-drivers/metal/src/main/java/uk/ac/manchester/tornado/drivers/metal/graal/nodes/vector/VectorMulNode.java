...
public class VectorMulNode extends FloatingNode implements LIRLowerable, VectorOp {
    ...
    public VectorMulNode(MetalKind kind, ValueNode x, ValueNode y) {
        this(TYPE, kind, x, y);
    }

    protected VectorMulNode(NodeClass<? extends VectorMulNode> c, MetalKind kind, ValueNode x, ValueNode y) {
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitVectorMul: %s + %s", input1, input2);
        gen.getLIRGeneratorTool().append(new AssignStmt(result, new MetalBinary.Expr(MetalBinaryOp.MUL, gen.getLIRGeneratorTool().getLIRKind(stamp), input1, input2)));
