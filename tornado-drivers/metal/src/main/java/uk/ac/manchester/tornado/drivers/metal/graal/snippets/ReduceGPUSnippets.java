...
/**
 * Tornado-Graal snippets for GPUs reductions using Metal semantics.
 */
public class ReduceGPUSnippets implements Snippets {
    ...
    public static void partialReduceIntAdd(int[] inputArray, int[] outputArray, int gidx) {
        ...
        int localIdx = MetalIntrinsics.get_local_id(0);
        int localGroupSize = MetalIntrinsics.get_local_size(0);
        int groupID = MetalIntrinsics.get_group_id(0);
    ...
    public static class Templates extends AbstractTemplates implements TornadoSnippetTypeInference {
        ...
        public SnippetInfo inferIntSnippet(ValueNode value, ValueNode extra) {
            ...
            if (value instanceof TornadoReduceAddNode) {
                ...
                MetalIntBinaryIntrinsicNode op = (MetalIntBinaryIntrinsicNode) value;
                snippet = getSnippetFromMetalBinaryNodeInteger(op, extra);
            } else {
                throw new RuntimeException("Reduce Operation no supported yet: snippet not installed");
            }
            return snippet;
        ...
        public SnippetInfo inferLongSnippet(ValueNode value, ValueNode extra) {
            ...
            if (value instanceof TornadoReduceAddNode) {
                ...
                MetalIntBinaryIntrinsicNode op = (MetalIntBinaryIntrinsicNode) value;
                snippet = getSnippetFromMetalBinaryNodeLong(op, extra);
            } else {
                throw new RuntimeException("Reduce Operation no supported yet: snippet not installed");
            }
            return snippet;
        ...
        private SnippetInfo getSnippetFromMetalBinaryNodeInteger(MetalFPBinaryIntrinsicNode value, ValueNode extra) {
            switch (value.operation()) {
                case FMAX:
        ...
        public SnippetInfo inferFloatSnippet(ValueNode value, ValueNode extra) {
            ...
            if (value instanceof TornadoReduceAddNode) {
                ...
                snippet = getSnippetFromMetalBinaryNodeInteger((MetalFPBinaryIntrinsicNode) value, extra);
            } else {
                throw new RuntimeException("Reduce Operation no supported yet: snippet not installed");
            }
            return snippet;
