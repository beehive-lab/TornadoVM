...
public class MetalArithmeticTool extends ArithmeticLIRGenerator {
    ...
    public Value emitFloatConvert(FloatConvert floatConvert, Value input) {
        ...
        switch (floatConvert) {
            case I2D:
                ...
                return emitUnaryAssign(MetalUnaryOp.CAST_TO_DOUBLE, LIRKind.value(MetalKind.DOUBLE), asSignedOperand(input, JavaKind.Int.getBitCount()));
            default:
    ...
    private Value asSignedOperand(Value x, int bits) {
        if (x.getValueKind() == null || !(x.getPlatformKind() instanceof MetalKind kind) || !kind.isUnsigned()) {
            return x;
        }
