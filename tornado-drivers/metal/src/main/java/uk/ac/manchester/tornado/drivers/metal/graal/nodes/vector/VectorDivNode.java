...
public class VectorDivNode extends FloatingNode implements LIRLowerable, VectorOp {
    ...
    public VectorDivNode(MetalKind kind, ValueNode x, ValueNode y) {
        this(TYPE, kind, x, y);
    }

    protected VectorDivNode(NodeClass<? extends VectorDivNode> c, MetalKind kind, ValueNode x, ValueNode y) {
    ...
    public void generate(NodeLIRBuilderTool gen) {
        ...
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitVectorDiv: %s + %s", input1, input2);
        gen.getLIRGeneratorTool().append(new AssignStmt(result, new MetalBinary.Expr(MetalBinaryOp.DIV, gen.getLIRGeneratorTool().getLIRKind(stamp), input1, input2)));
