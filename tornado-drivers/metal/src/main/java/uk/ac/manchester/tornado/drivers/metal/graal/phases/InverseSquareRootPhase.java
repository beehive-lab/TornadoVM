...
import uk.ac.manchester.tornado.drivers.metal.graal.nodes.RSqrtNode;
        ...
        graph.getNodes().filter(FloatDivNode.class).forEach(floatDivisionNode -> {
            ...
            if (floatDivisionNode.getX() instanceof ConstantNode constant) {
                if ((constant.getValue().toValueString().equals(ONE))) {
                    ...
                    } else if ((floatDivisionNode.getY() instanceof MetalFPUnaryIntrinsicNode MetalFPUnaryIntrinsicNode)) {
                        if (MetalFPUnaryIntrinsicNode.getOperation().equals(SQRT)) {
                            ValueNode n = MetalFPUnaryIntrinsicNode.getValue();
                            RSqrtNode rsqrtNode = new RSqrtNode(n);
                            graph.addOrUnique(rsqrtNode);
                            MetalFPUnaryIntrinsicNode.removeUsage(floatDivisionNode);
                            if (MetalFPUnaryIntrinsicNode.hasNoUsages()) {
                                MetalFPUnaryIntrinsicNode.safeDelete();
                            }
                            floatDivisionNode.replaceAtUsages(rsqrtNode);
                            floatDivisionNode.safeDelete();
                        }
