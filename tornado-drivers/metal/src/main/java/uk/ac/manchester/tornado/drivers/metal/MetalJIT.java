...
public class MetalJIT {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            final MetalBackend backend = getTornadoRuntime().getBackend(MetalBackendImpl.class).getDefaultBackend();

            TaskDataContext meta = TaskDataContext.create(new ScheduleContext("s0"), methodName, method);

            MetalCompilationResult result = MetalCompiler.compileCodeForDevice(resolvedMethod, new Object[] {}, meta, (MetalProviders) backend.getProviders(), backend, new EmptyProfiler());

            final long executionPlanId = 0;

            TornadoDevice device = TornadoCoreRuntime.getTornadoRuntime().getBackend(MetalBackendImpl.class).getDefaultDevice();
            MetalDeviceContext deviceContext = (MetalDeviceContext) device.getDeviceContext();
            MetalInstalledCode code = deviceContext.installCode(executionPlanId, result);
