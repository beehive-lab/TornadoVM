...
public abstract class MetalArrayWrapper<T> implements XPUBuffer {
    ...
    protected MetalArrayWrapper(final MetalDeviceContext device, final JavaKind kind, long batchSize, Access access) {
    ...
    protected MetalArrayWrapper(final T array, final MetalDeviceContext device, final JavaKind kind, long batchSize, Access access) {
     ...
     */
    private MetalByteBuffer buildArrayHeader(final int arraySize) {
        final MetalByteBuffer header = getArrayHeader();
    ...
    private MetalByteBuffer buildArrayHeaderBatch(final long arraySize) {
        final MetalByteBuffer header = getArrayHeader();
        for (int i = 0; i < arrayLengthOffset; i++) {
            header.buffer.put((byte) 0);
        }
    ...
    private MetalByteBuffer getArrayHeader() {
        final MetalByteBuffer header = new MetalByteBuffer(deviceContext, bufferId, bufferOffset, arrayHeaderSize);
        header.buffer.clear();
        return header;
    }
     ...
     */
    private MetalByteBuffer prepareArrayHeader() {
        final MetalByteBuffer header = getArrayHeader();
    ...
    private boolean validateArrayHeader(long executionPlanId, final T array) {
        final MetalByteBuffer header = prepareArrayHeader();
