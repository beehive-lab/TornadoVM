...
import uk.ac.manchester.tornado.drivers.metal.graal.nodes.CastNode;
 ...
 * TornadoVM Mid-IR).
 */
public class MetalLoweringProvider extends DefaultJavaLoweringProvider {
    ...
    public MetalLoweringProvider(MetaAccessProvider metaAccess, ForeignCallsProvider foreignCalls, PlatformConfigurationProvider platformConfig, MetaAccessExtensionProvider metaAccessExtensionProvider,
            ConstantReflectionProvider constantReflection, TornadoVMConfigAccess vmConfig, MetalTargetDescription target) {
    ...
    /**
     * {@link MetalLoweringProvider#gpuSnippet} is set during the lowering phase.
    ...
    public boolean supportsBulkZeroing() {
        unimplemented("MetalLoweringProvider::supportsBulkZeroing unimplemented");
        return false;
    }
    ...
    public void lowerLoadIndexedNode(LoadIndexedNode loadIndexed, LoweringTool tool) {
        ...
        if (!(loadIndexed.stamp(NodeView.DEFAULT) instanceof MetalStamp)) {
    ...
    public void lowerStoreIndexedNode(StoreIndexedNode storeIndexed, LoweringTool tool) {
        ...
        // compact-header-dependent Java-array base offset — so pass ARRAY_HEADER explicitly. Mirrors OCLLoweringProvider.
    ...
    private void lowerLocalNewArray(StructuredGraph graph, int length, NewArrayNonVirtualizableNode newArray) {
        ...
        localArrayNode = graph.addWithoutUnique(new LocalArrayNode(MetalArchitecture.localSpace, newArray.elementType(), newLengthNode));
    ...
    private AddressNode createArrayAccess(StructuredGraph graph, LoadIndexedNode loadIndexed, JavaKind elementKind) {
        ...
        if (isLocalIDNode(loadIndexed) || isPrivateIDNode(loadIndexed)) {
            ...
            // access by one element. Use the explicit-base overload with ARRAY_HEADER (JDK-neutral). Mirrors
            // OCLLoweringProvider.
    ...
    private AbstractWriteNode createMemWriteNode(JavaKind elementKind, ValueNode value, ValueNode array, AddressNode address, StructuredGraph graph, StoreIndexedNode storeIndexed) {
        ...
        if (!(valueStamp instanceof MetalStamp) || !((MetalStamp) valueStamp).getMetalKind().isVector()) {
