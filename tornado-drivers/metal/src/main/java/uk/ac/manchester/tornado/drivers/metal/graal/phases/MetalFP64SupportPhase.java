...
public class MetalFP64SupportPhase extends Phase {

    private TornadoDeviceContext deviceContext;

    public MetalFP64SupportPhase(TornadoDeviceContext deviceContext) {
    ...
    private void checkStampForFP64Support(Stamp stamp) {
        ...
        if (isStampFP64Type && !deviceContext.isFP64Supported()) {
            throw new TornadoDeviceFP64NotSupported("The current Metal device (" + deviceContext.getDeviceName() + ") does not support FP64");
        }
    }
    ...
    protected void run(StructuredGraph graph) {
        ...
        graph.getNodes().filter(MetalFPUnaryIntrinsicNode.class).forEach(node -> checkStampForFP64Support(node.stamp(NodeView.DEFAULT)));

        graph.getNodes().filter(MetalFPBinaryIntrinsicNode.class).forEach(node -> checkStampForFP64Support(node.stamp(NodeView.DEFAULT)));
