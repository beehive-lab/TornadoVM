 ...
 *
 * <p>The full set of shims is large (~160 lines) and most kernels use none of
 * them. While every shim is {@code inline} and the Metal front-end dead-strips
 * the unused ones (so the compiled pipeline is identical either way), emitting
 * all of them bloats {@code --printKernel} output and the MSL the driver must
 * parse at JIT time. {@link #buildFor(String)} therefore scans the generated
 * kernel source and emits only the sections whose identifiers actually appear,
 * always prefixed by the mandatory {@link #HEADER} and suffixed by the stable
 * {@link #PREAMBLE_END_MARKER}.
 *
 * <p>Why each section is needed:
 * <ul>
 *   <li><b>tornado_ptr_t</b> (always): {@code device uchar*} enables byte-level
 *       pointer arithmetic on device buffers; all address-computation variables
 *       (ul_N) use this type.</li>
 ...
 *       are then added so the types behave like native vector types. The
 *       vload8/16 and vstore8/16 overloads build on these.</li>
...
public final class MetalPreamble {
    ...
    /**
     * Stable marker terminating every emitted preamble. Callees inserted by
     * {@code MetalCompilationResult.addCompiledMethodCode} anchor on this so
     * their ordering is independent of which optional sections were emitted.
     */
    public static final String PREAMBLE_END_MARKER = "// ====== Tornado MSL preamble end ======";

    // @formatter:off

    /** Always emitted: include, namespace, and the byte-addressed device pointer type. */
    ...
    private static final String ATOMIC_ADD_FLOAT =
    ...
    // ── vload / vstore for n=2,3,4 (device + thread address spaces) ──────────────
    // MSL enforces strict address-space segregation: device* ≠ thread*
    private static final String VLOAD_STORE_2_3_4 =
        // float (device)
        ...
        // uchar (unsigned byte — ByteArray buffers, device)
        ...
        // char (signed byte — Int8Array / ImageByte kernels, device)
        ...
        // short (HalfFloatArray storage, device)
    ...
    private static final String ATOMIC_MUL_INT =
    ...
    /**
     * A preamble section emitted only when at least one of its trigger tokens
     * appears in the generated kernel source.
     */
    private record Section(String code, String... triggers) {
    }
    ...
    private static final Section[] SECTIONS = { //
            ...
            new Section(EXTENDED_VECTORS, "float8", "float16", "int8", "int16", "half8", "half16", "vload8", "vload16", "vstore8", "vstore16"), //
            new Section(ATOMIC_MUL_INT, "atomicMul_Tornado_Int"), //
    };

    /**
     * Builds the minimal preamble required to compile {@code kernelSource}: the
     * mandatory {@link #HEADER}, every section whose trigger tokens occur in the
     * source, and the trailing {@link #PREAMBLE_END_MARKER}.
     *
     * @param kernelSource
     *     the already-generated kernel text (signature, variable definitions and
     *     body), scanned to decide which optional shims are needed.
     * @return the preamble to prepend ahead of {@code kernelSource}.
     */
    public static String buildFor(String kernelSource) {
        StringBuilder sb = new StringBuilder(HEADER);
        for (Section section : SECTIONS) {
            if (isNeeded(kernelSource, section.triggers())) {
                sb.append(section.code());
            }
        }
        sb.append('\n').append(PREAMBLE_END_MARKER).append("\n\n");
        return sb.toString();
    }
    ...
    private static boolean isNeeded(String kernelSource, String[] triggers) {
        for (String trigger : triggers) {
            ...
            int idx = kernelSource.indexOf(trigger);
            while (idx >= 0) {
                int end = idx + trigger.length();
                boolean leftOk  = idx == 0 || (!Character.isLetterOrDigit(kernelSource.charAt(idx - 1)) && kernelSource.charAt(idx - 1) != '_');
                boolean rightOk = end == kernelSource.length() || (!Character.isLetterOrDigit(kernelSource.charAt(end)) && kernelSource.charAt(end) != '_');
                if (leftOk && rightOk) {
                    return true;
                }
                idx = kernelSource.indexOf(trigger, end);
            }
        }
        return false;
    }
