...
import uk.ac.manchester.tornado.drivers.metal.graal.lir.MetalGenTool;
...
import uk.ac.manchester.tornado.drivers.metal.graal.lir.MetalTernary;
import uk.ac.manchester.tornado.drivers.metal.graal.lir.MetalUnary;
import uk.ac.manchester.tornado.runtime.common.TornadoOptions;

/**
 * It traverses the Metal HIR and generates Metal LIR.
 */
public class MetalLIRGenerator extends LIRGenerator {
    ...
    public MetalLIRGenerator(CodeGenProviders providers, LIRGenerationResult res) {
        ...
        this.oclBuiltinTool = new MetalBuiltinTool();
        this.oclGenTool = new MetalGenTool(this);
    }

    public static MetalBinaryOp getConditionalOp(Condition condition) {
        switch (condition) {
            case AE:
            case GE:
                return MetalBinaryOp.RELATIONAL_GTE;
            case AT:
            case GT:
                return MetalBinaryOp.RELATIONAL_GT;

            case EQ:
                return MetalBinaryOp.RELATIONAL_EQ;

            case BE:
            case LE:
                return MetalBinaryOp.RELATIONAL_LTE;

            case BT:
            case LT:
                return MetalBinaryOp.RELATIONAL_LT;
            case NE:
                return MetalBinaryOp.RELATIONAL_NE;
            default:
                shouldNotReachHere();
                break;
    ...
    public LIRKind getLIRKind(Stamp stamp) {
        return (stamp instanceof MetalStamp) ? LIRKind.value(((MetalStamp) stamp).getMetalKind()) : super.getLIRKind(stamp);
    }

    @Override
    public Variable newVariable(ValueKind<?> valueKind) {
        PlatformKind pk = valueKind.getPlatformKind();
        if (!(pk instanceof MetalKind)) {
            shouldNotReachHere();
        }

        final Variable variable = super.newVariable(valueKind);
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "newVariable: %s <- %s (%s)", variable.toString(), valueKind.toString(), valueKind.getClass().getName());

        MetalLIRGenerationResult res = (MetalLIRGenerationResult) getResult();
    ...
    @Override
    public MetalLIRKindTool getLIRKindTool() {
        return (MetalLIRKindTool) super.getLIRKindTool();
    }
    ...
    public Variable emitConditionalMove(PlatformKind cmpKind, Value left, Value right, Condition cond, boolean unorderedIsTrue, Value trueValue, Value falseValue) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitConditionalMove: (%s %s %s) ? %s : %s, unorderedIsTrue:%s", left, cond, right, trueValue, falseValue, unorderedIsTrue);

        final MetalBinaryOp condOp = getConditionalOp(cond);
        final MetalBinary.Expr condExpr = new MetalBinary.Expr(condOp, LIRKind.value(cmpKind), left, right);
        final MetalTernary.Select selectExpr = new MetalTernary.Select(LIRKind.combine(trueValue, falseValue), condExpr, trueValue, falseValue);
    ...
    public void emitDeoptimize(Value actionAndReason, Value speculation, LIRFrameState state) {
        ...
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitDeoptimize: id=%d, reason=%s, action=%s", debugId, reason, action);
        append(new MetalControlFlow.DeoptOp(actionAndReason));
    }
    ...
    public Variable emitIntegerTestMove(Value left, Value right, Value trueValue, Value falseValue) {
        ...
        final MetalBinary.Expr condExpr = new MetalBinary.Expr(MetalBinaryOp.BITWISE_AND, null, left, right);
        final MetalTernary.Select selectExpr = new MetalTernary.Select(LIRKind.combine(trueValue, falseValue), condExpr, falseValue, trueValue);
    ...
    public void emitStrategySwitch(SwitchStrategy strategy, AllocatableValue key, LabelRef[] keyTargets, LabelRef defaultTarget) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitStrategySwitch: key=%s", key);
    ...
    public void emitReturn(JavaKind javaKind, Value input) {
        Logger.traceBuildLIR(Logger.BACKEND.Metal, "emitReturn: input=%s", input);
        if (input != null) {
            LIRKind lirKind = LIRKind.value(input.getPlatformKind());
            ExprStmt stmt = new ExprStmt(new MetalUnary.Expr(MetalUnaryOp.RETURN, lirKind, input));
            append(stmt);
        } else {
            append(new ExprStmt(new MetalNullary.Expr(MetalNullaryOp.RETURN, LIRKind.Illegal)));
        }
    }
    ...
    public MetalBuiltinTool getMetalBuiltinTool() {
        return oclBuiltinTool;
    }

    @Override
    public MetalTargetDescription target() {
        return (MetalTargetDescription) super.target();
    }
    ...
    @Override
    public MetalArithmeticTool getArithmetic() {
        return (MetalArithmeticTool) super.getArithmetic();
    }
