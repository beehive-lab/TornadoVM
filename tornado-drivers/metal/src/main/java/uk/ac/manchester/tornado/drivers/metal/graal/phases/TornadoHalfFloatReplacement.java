...
import uk.ac.manchester.tornado.drivers.metal.graal.nodes.vector.VectorAddHalfNode;
import uk.ac.manchester.tornado.drivers.metal.graal.nodes.vector.VectorLoadElementNode;
import uk.ac.manchester.tornado.drivers.metal.graal.nodes.vector.VectorMultHalfNode;
...
public class TornadoHalfFloatReplacement extends BasePhase<TornadoHighTierContext> {
    ...
    protected void run(StructuredGraph graph, TornadoHighTierContext context) {
        ...
        for (LoadFieldNode loadFieldNode : graph.getNodes().filter(LoadFieldNode.class)) {
            ...
            if (loadFieldNode.usages().filter(MetalConvertHalfToFloat.class).isNotEmpty()) {
        ...
        for (VectorValueNode vectorValueNode : graph.getNodes().filter(VectorValueNode.class)) {
            if (vectorValueNode.getMetalKind().isHalf()) {
                for (Node vectorElement : vectorValueNode.inputs()) {
                    if (vectorElement instanceof VectorLoadElementNode) {
                        ...
                        VectorLoadElementNode vectorLoadShort = new VectorLoadElementNode(MetalKind.HALF, vectorLoad.getVector(), vectorLoad.getLaneId());
    ...
    private static ValueNode getHalfOperand(ValueNode operand, StructuredGraph graph) {
        ...
        if (operand instanceof VectorLoadElementNode) {
            ...
            halfOperand = new VectorLoadElementNode(MetalKind.HALF, loadElementNodeX.getVector(), loadElementNodeX.getLaneId());
