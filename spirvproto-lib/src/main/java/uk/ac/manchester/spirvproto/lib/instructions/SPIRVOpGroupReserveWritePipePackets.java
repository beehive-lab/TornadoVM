...
public class SPIRVOpGroupReserveWritePipePackets extends SPIRVInstruction {
    ...
    public SPIRVOpGroupReserveWritePipePackets(SPIRVId _idResultType, SPIRVId _idResult, SPIRVId _execution, SPIRVId _pipe, SPIRVId _numPackets, SPIRVId _packetSize, SPIRVId _packetAlignment) {
        super(286, _idResultType.getWordCount() + _idResult.getWordCount() + _execution.getWordCount() + _pipe.getWordCount() + _numPackets.getWordCount() + _packetSize.getWordCount() + _packetAlignment.getWordCount() + 1, "OpGroupReserveWritePipePackets", SPIRVCapability.Pipes());
    ...
    public SPIRVCapability[] getAllCapabilities() {
        SPIRVCapability[] allCapabilities = new SPIRVCapability[this.capabilities.length + _idResultType.getCapabilities().length + _idResult.getCapabilities().length + _execution.getCapabilities().length + _pipe.getCapabilities().length + _numPackets.getCapabilities().length + _packetSize.getCapabilities().length + _packetAlignment.getCapabilities().length];
        int capPos = 0;
    ...
    public boolean equals(Object other) {
        if (other instanceof SPIRVOpGroupReserveWritePipePackets) {
            SPIRVOpGroupReserveWritePipePackets otherInst = (SPIRVOpGroupReserveWritePipePackets) other;
