# Installing TornadoVM in Apple Mac OS X #

### Pre-requisites

  * Maven Version 3
  * CMake 3.6 (or newer)
  * OpenCL: GPUs and CPUs >= 1.2
  * GCC (GCC >= 5.5) or clang/LLVM (LLVM>=10.0.1)
  * Python 2.7 (>= 2.7.5)
...
This will generate a new Java binary into the `jdk1.8.0_<your_version>/product`, e.g., `jdk1.8.0_131/product`.

***Attention:*** Due to conflicts between the latest llvm-clang compiler in the Mac OS X and the current version of the JDK 1.8 used in TornadoVM,
the above Java binary cannot be built successfully. As temporary fix, it is provided a [binary](https://www.dropbox.com/s/2aguj98jg5b5yh4/jdk1.8.0_131-osx-10.11.6.tgxz?dl=0) compiled in an earlier Mac OS X version.
