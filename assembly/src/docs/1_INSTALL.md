...
| OS                         | Hardware support for each backend                                                                    |
| -------------------------- | ---------------------------------------------------------------------------------------------------- |
| CentOS >= 7.3              | Any OpenCL (GPUs and CPUs >= 1.2, FPGAs >= 1.0) or CUDA 9.0+ compatible device, Level-Zero >= 1.1.2  |
| Fedora >= 21               | Any OpenCL (GPUs and CPUs >= 1.2, FPGAs >= 1.0) or CUDA 9.0+ compatible device, Level-Zero >= 1.1.2  |
| Ubuntu >= 16.04            | Any OpenCL (GPUs and CPUs >= 1.2, FPGAs >= 1.0) or CUDA 9.0+ compatible device, Level-Zero >= 1.1.2  |
...
TornadoVM can be built with two compiler backends and is able to generate OpenCL, PTX and SPIR-V code. At least one
backend must be specified at build time to the `make` command:

**Important [SPIR-V Backend Configuration]** The SPIR-V backends requires extra-steps (such as the Level-Zero installation and configuration for TornadoVM). If you wish to build the SPIR-V backend, please follow the guidelines for [here](22_SPIRV_BACKEND_INSTALL.md).
