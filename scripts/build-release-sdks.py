...
SDKs built per platform:
  ...
  - Linux   : opencl, ptx, cuda, full   (JDK 21 and JDK 25, via sdkman Temurin)
  - Windows : opencl, ptx, cuda, full   (JDK 21 and JDK 25, --jdkXX-home required)

"full" means opencl+ptx+cuda combined into a single archive.
...
# opencl+ptx+cuda is labelled "full" automatically by bin/compile.
BUILDS = {
    "macos": [
        "opencl",
        "metal",
    ...
    for jdk_major, tag, jdk_home in validated:
        ...
        try:
            ...
            for backends in backends_for_platform:
                # Label mirrors bin/compile naming: opencl,ptx,cuda → full
                if set(backends.split(",")) == {"opencl", "ptx", "cuda"}:
                    backend_label = "full"
                else:
