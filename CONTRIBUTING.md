...
We check the PR and test it internally. Be aware we are a very small team. Thus, depending on the PR, it might take some time for us to review it since we check the PR with our regression tests and benchmarks for all backends (OpenCL/PTX/CUDA/Metal platforms) as well as with different drivers and Operating Systems. 
