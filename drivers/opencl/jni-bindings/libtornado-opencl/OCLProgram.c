...
JNIEXPORT void JNICALL Java_tornado_drivers_opencl_OCLProgram_getBinaries
  (JNIEnv *env, jclass clazz, jlong program_id, jlong num_devices, jbyteArray array){
	OPENCL_PROLOGUE;
...
//	    printf("opencl> here, return size=%lu\n",return_size);
//	    for(int i=0;i<num_devices;i++){
//	    	printf("opencl> device=%d, size=%lu\n",i,binarySizes[i]);
//	    }

	    unsigned char **binaries = malloc(sizeof(unsigned char *) * num_devices);
//	    for(int i=0;i<num_devices;i++)
//	    	binaries[i] = value + binarySizes[i];
//
//	    OPENCL_SOFT_ERROR("clGetProgramInfo",
