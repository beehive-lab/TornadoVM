...
public class OCLNodeLIRBuilder extends NodeLIRBuilder {
    ...
    public void emitInvoke(Invoke x) {
        ...
        LIRKind lirKind = gen.getLIRKind(x.asNode().stamp());
        if (x.asNode().stamp().javaType(gen.getMetaAccess())
                .getAnnotation(Vector.class) != null) {
            lirKind = LIRKind.value(VectorKind.fromResolvedJavaType(x.asNode()
                    .stamp().javaType(gen.getMetaAccess())));
        }

        AllocatableValue result = gen.newVariable(lirKind);
...
//            System.out.printf("result: %s\n", result);
    ...
    public void emitIf(final IfNode x) {
        ...
        /**
         * test to see if this is an exception check
         * need to implement this properly? or omit!
         */
