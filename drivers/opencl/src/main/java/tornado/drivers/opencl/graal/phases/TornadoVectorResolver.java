...
public class TornadoVectorResolver extends BasePhase<TornadoHighTierContext> {
	...
	protected void run(StructuredGraph graph, TornadoHighTierContext context) {
		...
		/*
		 * For some reason we generate a new vector value node when a parameter is used...
		 * ... GVN may not be working in this case ...
		 * ... for now we manually resolve the vector loads on parameters
		 */
...
//		printNodes(usages);
		...
		if (elementLoads == 0 && elementStores == 0) return;
		...
		//		vectorLength, elementLoads, elementStores);
			...
			final Map<Block,ValueNode> blockExitValues = new HashMap<Block,ValueNode>();
			...
			for(final Block defBlock : defBlocks){
				final ValueNode exitValue = (VectorValueNode) renameVariables(cfg, defBlock, usages, vector, vector);
			...
			for(final Block useBlock : useBlocks){
				...
				if(useBlock.getBeginNode() instanceof AbstractMergeNode){
					...
					Debug.dump(graph, "before rename variables...");
		...
		Debug.dump(graph, "After renaming vector=%s",vector);
	}
	...
	private ValueNode renameVariables(ControlFlowGraph cfg, Block block, List<Node> usages, VectorValueNode toReplace,
		...
		if(block == null) return toReplace;
		
//		System.out.printf("resolver: renaming variables block=%s, toReplace=%s, incoming=%s\n",block,toReplace,incomingValue);
		
		/*
		 * create an ordered list of vector operations... these may be overlapping/interleaved
		 */
			...
			for (int i = 0; i < operations.size(); i++) {
				...
				if (node instanceof VectorStoreElementProxyNode) {
					...
					if (!dirty.get(lane)) {
						...
						stores[lane] = node;
						/*
						 * case where all lanes of a vector are set - flush the load
						 */
						...
						if (dirty.cardinality() == length) {
							...
							for (final Node store : stores)
									if(store!= null) ordered.add(store);
							dirty.clear();
							break;
						}
					}
					...
					/*
					 * we can re-order loads as are there are no outstanding stores on this lane
					 */
					if (!dirty.get(lane)) {
					...
					//System.out.printf("resolver: found vector use node=%s\n", node);
					/*
					 * ensure all stores have been committed before outputting this op
					 */
					if (dirty.isEmpty()) {
		...
		while (!ordered.isEmpty()) {
			...
			if (node instanceof VectorLoadElementProxyNode) {
				if (current instanceof VectorValueNode) processLoadElement(ordered, toReplace,
	...
	private void processUse(Node use, VectorValueNode toReplace,
		...
		if(use instanceof VectorLoadNode ) return;
	...
	private Set<Block> findDefs(ControlFlowGraph cfg, VectorValueNode vector, List<Node> usages) {
		...
		for (Node node : usages) {
			if (node instanceof VectorStoreElementProxyNode) defs.add(cfg.blockFor(node));
		}
		return defs;
	}
