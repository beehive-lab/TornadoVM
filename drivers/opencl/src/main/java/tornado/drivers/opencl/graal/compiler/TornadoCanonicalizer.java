...
//			System.out.printf("simplify: node=%s\n",node);
	...
	private void simplfyVectorValueNode(VectorValueNode node, SimplifierTool tool) {	
		if(node.getElement(0) instanceof VectorValueNode){
			...
			if(origin.getVectorKind() == node.getVectorKind()){
		...
		if(numInputs == node.getVectorKind().getVectorLength()){
...
//		System.out.printf("simplifyVectorOp: node=%s, op=%s\n",node,op);
		...
		switch(op){
			case ADD:
				final VectorAddNode addNode = node.graph().addOrUnique(new VectorAddNode(vectorA, vectorB));
				node.set(addNode);
				for(Node n : ops){
					//n.removeUsage(node);
				...
				break;
			case DIV:
				final VectorDivNode divNode = node.graph().addOrUnique(new VectorDivNode(vectorA, vectorB));
				node.set(divNode);
				for(Node n : ops){
					//n.removeUsage(node);
					GraphUtil.tryKillUnused(n);
				...
				break;
			case MULT:
				final VectorMulNode mulNode = node.graph().addOrUnique(new VectorMulNode(vectorA, vectorB));
				node.set(mulNode);
				for(Node n : ops){
					//n.removeUsage(node);
					GraphUtil.tryKillUnused(n);
			...
			case SUB:
				...
				final VectorSubNode subNode = node.graph().addOrUnique(new VectorSubNode(vectorA, vectorB));
				node.set(subNode);
				for(Node n : ops){
					//n.removeUsage(node);
					GraphUtil.tryKillUnused(n);
...
//		System.out.printf("baseOp: %s\n",baseOp);
		if(baseOp == VectorOp.ILLEGAL) return VectorOp.ILLEGAL;
		
		
		
		List<VectorLoadElementNode> loads = getVector(ops.get(0));
		
		final Set<VectorValueNode> vectors = new HashSet<VectorValueNode>();
		
		loads.forEach(load -> vectors.add(load.getVector()));
//		System.out.printf("vectors: size=%d\n",vectors.size());
		if(vectors.size() != 2) return VectorOp.ILLEGAL;
...
//		System.out.printf("lane conflicts=%s\n",conflicts);
		if(conflicts || loadIdx == -1 || !lanesA.get(loadIdx) || !lanesB.get(loadIdx)) return VectorOp.ILLEGAL;
...
//				System.out.printf("ops match\n");
				...
				if(loads.size() != 2) return VectorOp.ILLEGAL;
				
//				System.out.printf("size match\n");
				
				if(!vectors.contains(loads.get(0).getVector())  || !vectors.contains(loads.get(1).getVector())) return VectorOp.ILLEGAL;
...
//				System.out.printf("lane conflicts=%s\n",conflicts);
	...
	public enum VectorOp {MULT, ADD, SUB, DIV, ILLEGAL};
