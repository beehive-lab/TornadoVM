...
public class OCLScheduler {
	...
	public static final OCLKernelScheduler create(final OCLDeviceContext context){
		if(context.getDevice().getDeviceType() == OCLDeviceType.CL_DEVICE_TYPE_CPU || context.getDevice().getDeviceType() == OCLDeviceType.CL_DEVICE_TYPE_ACCELERATOR)
			return new OCLCpuScheduler(context);
		else if (context.getDevice().getDeviceType() == OCLDeviceType.CL_DEVICE_TYPE_GPU )
