...
public class VectorLoadNode extends AccessIndexedNode {
     ...
     */
    public VectorLoadNode(PTXKind kind, ValueNode array, ValueNode index) {
        super(TYPE, PTXStampFactory.getStampFor(kind), array, index, null, JavaKind.Illegal);
        this.kind = kind;
    }
    ...
    public PTXKind elementType() {
        return kind.getElementKind();
    }

    public PTXKind vectorKind() {
        return kind;
    }

    @Override
    public JavaKind elementKind() {
        return ((PTXStamp) stamp(NodeView.DEFAULT)).getPTXKind().getElementKind().asJavaKind();
    }
}
