...
public class CUDAObjectWrapper implements ObjectBuffer {
    ...
    public CUDAObjectWrapper(final CUDADeviceContext device, Object object, long batchSize) {
        ...
        for (HotSpotResolvedJavaField field : fields) {
            ...
            if (type.isArray()) {
                if (type == int[].class) {
                    wrappedField = new CUDAIntArrayWrapper(device);
                } else if (type == float[].class) {
                    wrappedField = new CUDAFloatArrayWrapper(device);
                } else if (type == double[].class) {
                    wrappedField = new CUDADoubleArrayWrapper(device);
                } else if (type == long[].class) {
                    wrappedField = new CUDALongArrayWrapper(device);
                } else if (type == short[].class) {
                    wrappedField = new CUDAShortArrayWrapper(device);
                } else if (type == byte[].class) {
                    wrappedField = new CUDAByteArrayWrapper(device);
                } else {
                ...
                // lambda expression
                try {
                    wrappedField = new CUDAObjectWrapper(device, reflectedField.get(object), batchSize);
