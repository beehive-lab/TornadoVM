...
public class Benchmark {
	...
	private static final String	BENCHMARK_NAME	= "dot-vector";
	...
	public static void run(int iterations, int numElements) {
		...
		final DotJava referenceTest = new DotJava(iterations, numElements);
		...
		final DotTornadoDummy tornadoOverhead = new DotTornadoDummy(iterations, numElements);
		...
		for (int platformIndex = 0; platformIndex < oclRuntime.getNumPlatforms(); platformIndex++) {
			for (int deviceIndex = 0; deviceIndex < oclRuntime.getNumDevices(platformIndex); deviceIndex++) {
				...
				final DotTornado deviceTest = new DotTornado(iterations, numElements, device);
