 ...
 *
 * <p>How to run: {@code tornado-test -V uk.ac.manchester.tornado.unittests.arrays.TestHalfFloatShortBitsOpenCL}</p>
 */
public class TestHalfFloatShortBitsOpenCL extends TornadoTestBase {
    ...
    private void assumeOpenCLBackend() {
        ...
        if (backendType != TornadoVMBackendType.OPENCL) {
            String message = "This HalfFloat raw-bits fix targets the OpenCL backend (default device is " + backendType + ")";
            switch (backendType) {
    ...
    public void testStoreBits() throws TornadoExecutionPlanException {
        ...
        TaskGraph tg = new TaskGraph("sb").transferToDevice(DataTransferMode.FIRST_EXECUTION, bits) //
                .task("t0", TestHalfFloatShortBitsOpenCL::storeBits, bits, out) //
    ...
    public void testRoundTripBits() throws TornadoExecutionPlanException {
        ...
        TaskGraph tg = new TaskGraph("rt").transferToDevice(DataTransferMode.FIRST_EXECUTION, in) //
                .task("t0", TestHalfFloatShortBitsOpenCL::roundTripBits, in, out) //
                .transferToHost(DataTransferMode.EVERY_EXECUTION, out);
        try (TornadoExecutionPlan plan = new TornadoExecutionPlan(tg.snapshot())) {
            plan.execute();
        }
    ...
    public void testStoreBitsKernelContext() throws TornadoExecutionPlanException {
        ...
        TaskGraph tg = new TaskGraph("kc").transferToDevice(DataTransferMode.EVERY_EXECUTION, bits, out) //
                .task("t0", TestHalfFloatShortBitsOpenCL::storeBitsKernelContext, new KernelContext(), bits, out) //
                .transferToHost(DataTransferMode.EVERY_EXECUTION, out);
    ...
    public void testStageBitsThroughLocalTile() throws TornadoExecutionPlanException {
        ...
        TaskGraph tg = new TaskGraph("tl").transferToDevice(DataTransferMode.EVERY_EXECUTION, bits, out) //
                .task("t0", TestHalfFloatShortBitsOpenCL::stageBitsThroughLocalTile, new KernelContext(), bits, out) //
                .transferToHost(DataTransferMode.EVERY_EXECUTION, out);
    ...
    public void testConstantBits() throws TornadoExecutionPlanException {
        ...
        TaskGraph tg = new TaskGraph("cb") //
                .task("t0", TestHalfFloatShortBitsOpenCL::constantBits, out) //
                .transferToHost(DataTransferMode.EVERY_EXECUTION, out);
        try (TornadoExecutionPlan plan = new TornadoExecutionPlan(tg.snapshot())) {
            plan.execute();
        }
