 ...
 */
public class TestLLMBufferStateSharing2 extends TornadoTestBase {
    ...
    public static void matrixVectorSimple(FloatArray in, FloatArray out, FloatArray weights, int dim) {
        // Simple operation to simulate matrix multiplication
        for (int i = 0; i < dim; i++) {
            float sum = 0.0f;
            for (int j = 0; j < Math.min(dim, in.getSize()); j++) {
                sum += in.get(j);
            }
            // Simple calculation that doesn't depend on accessing out-of-bounds elements
    ...
    public void testOriginalImplementation() throws TornadoExecutionPlanException {
        ...
        // Execute and test first two graphs first
        ...
        // Create a separate plan for the third graph to avoid any issues
        ...
        try (TornadoExecutionPlan qkvPlan = new TornadoExecutionPlan(qkvGraph.snapshot())) {
     ...
     * Split into multiple execution plans to avoid dependencies issues
     */
    @Test
    public void testFullPipelineTouchBuffer() throws TornadoExecutionPlanException {
        ...
        // First execute lookUpBufferX and rmsNormGraph
        try (TornadoExecutionPlan executionPlan =
                new TornadoExecutionPlan(
                        lookUpBufferX.snapshot(),
                        rmsNormGraph.snapshot())) {
        ...
        // QKV Matmuls graph - run separately to avoid the Index out of bounds issue
        TaskGraph qkvGraph = new TaskGraph("qkv")
                .transferToDevice(DataTransferMode.EVERY_EXECUTION, state.wrapXb, state.positionAndLayer)
                .transferToDevice(DataTransferMode.FIRST_EXECUTION, weights)
                .task("qmatmul", TestLLMBufferStateSharing::matrixVectorSimple,
