...
TornadoVM targets **OpenCL, CUDA, Metal and SPIR-V** compatible devices which include multi-core CPUs, dedicated
GPUs (Intel, NVIDIA, AMD), integrated GPUs (Apple Silicon, Intel HD Graphics and ARM Mali), and FPGAs (Intel and Xilinx).

TornadoVM has four backends that generate OpenCL C, CUDA C (compiled to PTX via NVRTC), SPIR-V binary, and Metal Shading Language.
