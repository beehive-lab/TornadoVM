...
public class TornadoMath {
	...
	public static boolean isEqual(float[] a, float[] b){
		...
		for(int i=0;i<a.length && result;i++){
			result = FloatOps.compareBits(a[i], b[i]);
		}
		return result;
	}
	...
	public static boolean isEqualULP(float[] value, float[] expected, float numULP){
		...
		for(int i=0;i<value.length && result;i++){
			result = FloatOps.compareULP(value[i], expected[i], numULP);
		}
		return result;
	}
	...
	public static boolean isEqualTol(float[] a, float[] b, float tol){
		boolean result = true;
		for(int i=0;i<a.length && result;i++){
			result = FloatOps.compare(a[i], b[i], tol);
		}
		return result;
	}
	...
	public static String bitDiff(float[] a, float[] b){
		...
		for(int i=0;i<a.length;i++){
			sb.append(Integer.toBinaryString(Float.floatToIntBits(a[i]) ^ Float.floatToIntBits(b[i])));
			sb.append(" ");
		}
		return sb.toString();
	}
