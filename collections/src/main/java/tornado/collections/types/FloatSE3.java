...
public class FloatSE3 {
	...
	final Float3 translation = new Float3();
	...
	public FloatSE3(Float6 v){
		...
		/*Matrix.set(matrix,1,0, -v[2]);
		Matrix.set(matrix,2,0, v[1]);
		Matrix.set(matrix,2,1, -v[0]);
		
		Matrix.set(matrix,0,1, v[2]);
		Matrix.set(matrix,0,2, -v[1]);
		Matrix.set(matrix,1,2, v[0]);
		
		Matrix.set(matrix,3,0, v[3]);
		Matrix.set(matrix,3,1, v[4]);
	...
	public void exp(Float6 mu){
		...
		if(theta_sq < 1e-8f){
			...
			B = 0.5f;			
			Float3.add(mu_lo,Float3.mult(crossProduct, 0.5f),translation);
		} else {
			if(theta_sq < 1e-6f){
			...
			Float3.add(mu_lo, Btmp, translation);
...
//	public MatrixFloat getRotation(){
//		MatrixFloat result = new MatrixFloat(3,3);
//
//		for(int r=0;r<3;r++)
//			for(int c=0;c<3;c++){
...
//	}
	...
	private final float sq(float value){
		return value * value;
	}
