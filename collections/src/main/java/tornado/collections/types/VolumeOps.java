...
import static tornado.collections.types.Int3.*;
...
public class VolumeOps {
	...
	public static final Float3 grad(final VolumeShort2 volume,
		...
		final Float3 scaledPos = new Float3(
				((point.getX() * (float) volume.X()) / dim.getX()) - 0.5f,
				((point.getY() * (float) volume.Y()) / dim.getY()) - 0.5f,
				((point.getZ() * (float) volume.Z()) / dim.getZ()) - 0.5f );
		...
		final float gx = ((((
				...
				(vs(volume, upperLower.getX(),lower.getY(), lower.getZ())) - vs(volume, lowerLower.getX(), lower.getY(), lower.getZ()))  * (1 - factor.getX()) 
		...
		final float gy = ((((((
				vs(volume, lower.getX(),upperLower.getY(), lower.getZ()) 
		...
		final float gz = ((((((
				...
				vs(volume, lower.getX(),lower.getY(), upperLower.getZ()))
		...
		final Float3 tmp1 = mult(
				new Float3(dim.getX() / (float) volume.X(),
				dim.getY() / (float) volume.Y(), 
				dim.getZ() / (float) volume.Z()), (0.5f * 0.00003051944088f));
