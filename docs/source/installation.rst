...
Most users do not need to build TornadoVM from source. Prebuilt SDKs (OpenCL, CUDA, SPIR-V, Metal, or a full bundle) are available from the official website:
...
To install a specific JDK and/or backend combination, pass the candidate version as ``<version>-<jdk-version>-<backend>`` (e.g. ``opencl``, ``cuda``, ``spirv``, ``metal``, or ``full`` for all backends). Run ``sdk list tornadovm`` to see all available combinations.
