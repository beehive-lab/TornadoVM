...
TornadoVM does not replace existing VMs, but it rather complements them with the capability of offloading Java code to OpenCL, CUDA and Metal,
...
-   **Hardware agnostic APIs:** The APIs are hardware agnostic, which means that, from the developers' view, the source code is exactly the same for CPUs and GPUs. It is the TornadoVM runtime and the TornadoVM JIT Compiler that specialises the code per architecture. To express parallelism within a method, developers use either the :ref:`Loop Parallel API <loop-parallel-api>` (Java annotations such as ``@Parallel``, recommended for non-experts) or the lower-level :ref:`KernelContext API <kernel-context-api>` (``KernelContext``, for developers who want explicit control over thread IDs, local memory and barriers), or the :ref:`TileContext API <tile-context-api>` (``TileContext``, NVIDIA CUDA backend only, for expressing the computation over tiles of a tensor and letting the CUDA Tile compiler pick the thread mapping and the tensor-core instructions). Methods prepared with any of these APIs are then grouped and orchestrated with the :ref:`Task-Graph API <task-graph-api>` together with the :ref:`TornadoExecutionPlan <execution-plan>`: the ``TaskGraph`` defines *what* to run (the data movement and the methods to accelerate), while the ``TornadoExecutionPlan`` defines *how* to run it on the target device (e.g., with the profiler enabled, with batch processing enabled, on a specific device, etc.).
-   **Optimising Just In Time (JIT) compiler per device architecture:** This means that the code is specialised per architecture (e.g., the GPU generated code is specialised differently compared to CPUs).
