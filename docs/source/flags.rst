...
.. table::
   ...
   ``-Dtornado.cuda.nvrtc.library=PATH``                             Loads this NVRTC ahead of every search heuristic, so a host holding several CUDA toolkits can pin the one it means to compile with. The ``TORNADO_NVRTC_LIBRARY`` environment variable does the same; the property wins when both are set, and an unloadable path falls back to the normal search. Which NVRTC was loaded is named in the ``sm_XX`` fallback warning.
