...
public class RuntimeUtilities {
    ...
    public static String humanReadableBPS(double bps) {
        final int unit = 1000;
        if (bps < unit) {
            return String.format("%.2f bps", bps);
        }
        final int exp = (int) (Math.log(bps) / Math.log(unit));
        final char pre = "KMGTPE".charAt(exp - 1);
        return String.format("%.1f %sbps", bps / Math.pow(unit, exp), pre);
    }
    ...
    public static int getArrayDims(Class<?> type) {
